...
var
  fmListZonas: TfmListZonas;

implementation

uses CZonasCCB, CUtiles, tabladezonas;

{$R *.DFM}

procedure TfmListZonas.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if not introSalir then Begin
    zonas.desconectar;
    Release; fmListZonas := nil;
  end;
end;

procedure TfmListZonas.expresionChange(Sender: TObject);
begin
  if criterio.Text = 'Descripción' then zonas.BuscarPorZona(expresion.Text);
  if criterio.Text = 'Código'      then zonas.BuscarPorId(expresion.Text);
end;

procedure TfmListZonas.criterioClick(Sender: TObject);
begin
  ActiveControl := expresion;
end;

procedure TfmListZonas.expresionKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmListZonas.SpeedButton1Click(Sender: TObject);
begin
  Application.CreateForm(TfmTablaZonas, fmTablaZonas);
  fmTablaZonas.AltaClick(nil);
  fmTablaZonas.zona.Text     := expresion.Text;
  fmTablaZonas.ActiveControl := fmTablaZonas.zona;
  fmTablaZonas.NoCerrarFinal := True;
  fmTablaZonas.ShowModal;
end;

procedure TfmListZonas.SpeedButton2Click(Sender: TObject);
begin
  Application.CreateForm(TfmTablaZonas, fmTablaZonas);
  fmTablaZonas.NoCerrarFinal := True;
  fmTablaZonas.idzona.Text   := zonas.tabla.FieldByName('idzona').AsString;
  fmTablaZonas.ShowModal;
end;

procedure TfmListZonas.DBGridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_INSERT then SpeedButton1Click(Sender);
   ...
   if introSalir then Begin
     ...
     if Not Panel3.Visible then SpeedButton2Click(Sender) else Begin
        if control = 0 then Begin    // El control es del id
          if codigo.Checked then desde.Text := zonas.tabla.FieldByName('idzona').AsString else desde.Text := zonas.tabla.FieldByName('zona').AsString;
          ActiveControl := hasta;
        end;
        if control = 1 then Begin    // El control es de la descripción
          if codigo.Checked then hasta.Text := zonas.tabla.FieldByName('idzona').AsString else hasta.Text := zonas.tabla.FieldByName('zona').AsString;
...
procedure TfmListZonas.criterioChange(Sender: TObject);
begin
  expresion.Text := '';
  expresionChange(Sender);
end;

procedure TfmListZonas.SpeedButton4Click(Sender: TObject);
begin
...
procedure TfmListZonas.codigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := desde;
end;

procedure TfmListZonas.codigoClick(Sender: TObject);
begin
  criterio.Text := 'Código';
  expresionChange(Sender);
...
procedure TfmListZonas.alfabeticoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := desde;
end;

procedure TfmListZonas.alfabeticoClick(Sender: TObject);
begin
...
procedure TfmListZonas.desdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then
...
procedure TfmListZonas.hastaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmListZonas.dispositivoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := emitir;
end;

procedure TfmListZonas.emitirClick(Sender: TObject);
var
...
begin
  if (Length(Trim(desde.Text)) = 0) or (Length(Trim(hasta.Text)) = 0) then utiles.msgError('No se han definido suficientes parámetros ...!') else Begin
    ...
    zonas.Listar(ordensalida, desde.Text, hasta.Text, entornosal, salida);
...
procedure TfmListZonas.cerrarClick(Sender: TObject);
begin
...
procedure TfmListZonas.FormShow(Sender: TObject);
begin
  if FormStyle = fsMDIChild then Begin
    Height := 380; Width := 571; Top := 25; Left:=(Screen.Width - Width) div 2;
  end;
  DTS.DataSet := zonas.tabla;
  if not introSalir then zonas.conectar;
end;

procedure TfmListZonas.desdeClick(Sender: TObject);
begin
  control := 0;
end;

procedure TfmListZonas.hastaClick(Sender: TObject);
begin
  control := 1;
end;

procedure TfmListZonas.SpeedButton3Click(Sender: TObject);
begin
  if not (introsalir) then begin
    Application.CreateForm(TfmTablaZonas, fmTablaZonas);
    fmTablaZonas.idzona.Text := zonas.tabla.FieldByName('idzona').AsString;
    fmTablaZonas.BajaClick(Sender);
