...
var
  fmListMedicos: TfmListMedicos;

implementation

uses CMedicosCCB, CUtiles, tablademedicoss;

{$R *.DFM}

procedure TfmListMedicos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if not introSalir then Begin
    medico.desconectar;
    Release; fmListMedicos := nil;
  end;
end;

procedure TfmListMedicos.expresionChange(Sender: TObject);
begin
  if criterio.Text = 'Nombre' then medico.BuscarPorNombre(expresion.Text);
  if criterio.Text = 'Código' then medico.BuscarPorId(expresion.Text);
end;

procedure TfmListMedicos.criterioClick(Sender: TObject);
begin
  ActiveControl := expresion;
end;

procedure TfmListMedicos.expresionKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmListMedicos.SpeedButton1Click(Sender: TObject);
begin
...
procedure TfmListMedicos.SpeedButton2Click(Sender: TObject);
begin
  Application.CreateForm(TfmTablaMedicos, fmTablaMedicos);
  fmTablaMedicos.NoCerrarFinal := True;
  fmTablaMedicos.idprof.Text   := medico.tabla.FieldByName('idprof').AsString;
  fmTablaMedicos.ShowModal;
end;

procedure TfmListMedicos.FormCreate(Sender: TObject);
begin
  Left:=(Screen.Width - Width) div 2; Top:=(Screen.Height - Height) div 2;
end;

procedure TfmListMedicos.DBGridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_INSERT then SpeedButton1Click(Sender);
   ...
   if introSalir then Begin
     ...
     if Not Panel3.Visible then SpeedButton2Click(Sender) else Begin
        if control = 0 then Begin    // El control es del id
          if codigo.Checked then desde.Text := medico.tabla.FieldByName('idprof').AsString else desde.Text := medico.tabla.FieldByName('nombre').AsString;
          ActiveControl := hasta;
        end;
        if control = 1 then Begin    // El control es de la descripción
          if codigo.Checked then hasta.Text := medico.tabla.FieldByName('idprof').AsString else hasta.Text := medico.tabla.FieldByName('nombre').AsString;
...
procedure TfmListMedicos.criterioChange(Sender: TObject);
begin
  expresion.Text := '';
  expresionChange(Sender);
end;

procedure TfmListMedicos.SpeedButton4Click(Sender: TObject);
begin
...
procedure TfmListMedicos.codigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := desde;
end;

procedure TfmListMedicos.codigoClick(Sender: TObject);
begin
  criterio.Text := 'Código';
  expresionChange(Sender);
...
procedure TfmListMedicos.alfabeticoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := desde;
end;

procedure TfmListMedicos.alfabeticoClick(Sender: TObject);
begin
...
procedure TfmListMedicos.desdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then
...
procedure TfmListMedicos.hastaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmListMedicos.dispositivoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then ActiveControl := emitir;
end;

procedure TfmListMedicos.emitirClick(Sender: TObject);
var
...
begin
  if (Length(Trim(desde.Text)) = 0) or (Length(Trim(hasta.Text)) = 0) then utiles.msgError('No se han definido suficientes parámetros ...!') else Begin
    ...
    medico.Listar(ordensalida, desde.Text, hasta.Text, entornosal, salida);
    StatusBar1.Panels[0].Text := '';
  end;
  ActiveControl := cerrar;
end;

procedure TfmListMedicos.cerrarClick(Sender: TObject);
begin
...
procedure TfmListMedicos.FormShow(Sender: TObject);
begin
  if FormStyle = fsMDIChild then Begin
    Height := 380; Width := 571; Top := 25; Left:=(Screen.Width - Width) div 2;
  end;
  DTS.DataSet := medico.tabla;
  if not introSalir then medico.conectar;
  expresionChange(Self);
end;

procedure TfmListMedicos.desdeClick(Sender: TObject);
begin
  control := 0;
end;

procedure TfmListMedicos.hastaClick(Sender: TObject);
begin
  control := 1;
end;

procedure TfmListMedicos.SpeedButton3Click(Sender: TObject);
begin
  if not (introsalir) then begin
    Application.CreateForm(TfmTablaMedicos, fmTablaMedicos);
    fmTablaMedicos.idprof.Text := medico.tabla.FieldByName('idprof').AsString;
