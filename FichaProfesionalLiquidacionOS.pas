unit FichaProfesionalLiquidacionOS;
...
type
  TfmFichaProfesionalCCB = class(TForm)
    ...
    procedure globalKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
...
procedure TfmFichaProfesionalCCB.CheckBox1Click(Sender: TObject);
begin
  profesional.Estado(idprof.Text, checkbox1.Checked);
end;
...
procedure TfmFichaProfesionalCCB.globalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then ActiveControl := periodo;
  if (Key = VK_RETURN) or (Key = VK_DOWN) then begin
    if not utiles.Sionoct(retieneiva.Text, 'SN', 'Las Opciones son S o N ...!') then exit;
    if (Length(Trim(idprof.Text)) > 0) and (Length(Trim(nombre.Text)) > 0) and (categoria.Buscar(idcategoria.Text)) and (utiles.Sionoct(ajustedc.Text, 'SN', '')) and (utiles.verificarPeriodo(periodo.Text, '')) and (utiles.Sionoct(global.Text, 'SN', '')) then
      if idprof.Text = codfact.Text then utiles.msgError('El Profesional No Puede Facturar para Sí Mismo ...!') else Begin
        if Not nivel2.Visible then profesional.Grabar(idprof.Text, nombre.Text, domicilio.Text, '', '', telefono.Text, nrocuit.Text, email.Text, idcategoria.Text, matricula.Text, fechamat.Text, ajustedc.Text, codfact.Text, nivel2.Text, retieneiva.Text, global.Text) else
          profesional.Grabar(idprof.Text, nombre.Text, domicilio.Text, '', '', telefono.Text, nrocuit.Text, email.Text, idcategoria.Text, matricula.Text, fechamat.Text, ajustedc.Text, codfact.Text, nivel2.Text, retieneiva.Text, global.Text);
...
begin
  ...
  if (Key = VK_RETURN) or (Key = VK_DOWN) then
   if utiles.verificarPeriodo(periodo.Text) then global.SetFocus;
end;
