...
type
  TRasDialerForm = class(TForm) //(TAutoForm)
    Panel1: TPanel;
    CheckTimer: TTimer;
    Label2: TLabel;
    Label3: TLabel;
    ...
    procedure BorrarLogs1Click(Sender: TObject);
  ...
  private
    ...
    inicioConnect, finConnect: string;
...
uses
    RasDial2, RasDial3, RasDial4, ControlAccesoTel, pasword, CUtiles;
...
function TRasDialerForm.setIPAdress: String;
Begin
...
begin
    ...
    CancelButton.Caption := '&Desconectar';
    CheckTimer.Enabled   := TRUE;
    Caption              := ProgName + ' - Connectado';
    inicioConnect        := TimeToStr(Now);
    {AutoForm.WindowState := wsMinimized;}
    cerrarDialer         := True;
    Close;
end;
...
begin
    ...
    CancelButton.Caption  := 'Ca&ncelar';
...
var
    ...
    Key         : String;
begin
    if Application.MessageBox('Borrar la duración acumulada ?', 'Warning',
...
begin
    ...
    Duracion := DurationLabel.Caption + TimeToStr(Frac(Duration));
...
//    SaveSettings;
    ...
    if not cerrarDialer then Begin
      if hRasConn <> 0 then begin
        Status := Application.MEssageBox('Desconectar antes de salir ?',
                                         'Warning',
                                         MB_YESNOCANCEL);
...
begin
    if ConnectTime = 0 then
        ...
        LogMessage('Desconectando');
        DoDuration;
    end;

    Disconnected;

    finConnect := TimeToStr(Now);

    // Guardamos el log
    control.Guardar(EntryNameComboBox.Text, DateToStr(Now), inicioConnect, finConnect, Duracion);
end;
...
begin
    ...
    FirstTime := True;   // Para que cada vez que se abre el form cargue usuario y contraseña
...
begin
    AboutForm.Label4.Caption := 'EMail: silmos@i2net.com.ar';
    AboutForm.ShowModal;
end;
...
begin
    ...
    if IPAddr > '' then
        ...
        InfoListBox.Items.Add('IP desconocido');
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}

procedure TRasDialerForm.Consultar1Click(Sender: TObject);
begin
  inherited;
  {Application.CreateForm(TfmPasword, fmPasword);
  fmPasword.clave_acceso := 'jramirez2001';
  fmPasword.ShowModal;
  if fmPasword.pwdok then control.Consultar else utiles.msgError('Contraseña incorrecta');
  fmPasword.Release; fmPasword := nil;}
  control.Consultar;
end;

procedure TRasDialerForm.BorrarLogs1Click(Sender: TObject);
begin
  inherited;
  Application.CreateForm(TfmPasword, fmPasword);
  fmPasword.clave_acceso := 'jramirez2001';
  fmPasword.ShowModal;
  if fmPasword.pwdok then Begin
    control.Borrar;
    MnuCleardurationClick(Sender);
  end else utiles.msgError('Contraseña incorrecta');
  fmPasword.Release; fmPasword := nil;
end;

procedure TRasDialerForm.FormActivate(Sender: TObject);
  var
    Duration    : Integer;
    IniFile     : TIniFile;
    Section     : String;
