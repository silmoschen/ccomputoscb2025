...
TTAgrupaOSFacturar = class
 ...
 public
  ...
  function    getListaObrasSocialesAgrupadas: TQuery;

  function    alta: string;
...
begin
  if (buscar(xid)) then begin
    tabla.Delete;
    datosdb.closeDB(tabla); tabla.Open;
    datosdb.tranSQL('delete from ' + detos.TableName + ' where id = ' + '''' + xid + '''');
    datosdb.refrescar(detos);
  end;
end;
...
function TTAgrupaOSFacturar.getListaObrasSocialesAgrupadas: TQuery;
begin
  result := datosdb.tranSQL('select * from ' + detos.TableName);
end;

function TTAgrupaOSFacturar.alta: string;
var
  rs: TQuery;
begin
  rs := datosdb.tranSQL('select max(id) from ' + tabla.TableName);
  rs.Open;
  if (rs.Fields[0].IsNull) then result := '1' else result := IntToStr( rs.Fields[0].AsInteger + 1 );
  rs.close; rs.Free;
end;
