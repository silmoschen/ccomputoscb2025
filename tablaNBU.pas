...
uses
  ...
  ComCtrls, ToolWin, Editv, Grids, Contnrs;
...
uses CNBU, CUtiles, ImgForms, CConfigForms, CNomeclaCCB, NominaNomeclador,
     CUtilidadesStringGrid, CUnidadesNBU;
...
begin
  ...
  unidades.Text    := utiles.FormatearNumero(FloatToStr(nbu.unidades));
  ...
  unidadesNBU.getUnidad(codigo.Text, utiles.setPeriodoActual);
  unidad.Text      := utiles.FormatearNumero(FloatToStr(unidadesNBU.getUnidades));
  unidades.Text    := utiles.FormatearNumero(FloatToStr(unidadesNBU.getUnidadDif));
  periodo.Text     := unidadesNBU.getPeriodo;
end;
...
Begin
  ...
  For i := 1 to l.Count do Begin
    ...
    S.Cells[1, i] := nomeclatura.descrip;
    Inc(items);
    S.Row := items;
  end;
  l.Destroy; l := Nil;
...
procedure TfmTablaNBU.CargarUnidades;
// Objetivo...: Cargar Unidades
var
  i, j: Integer;
  l: TObjectList;
...
Begin
  ...
  l := unidadesNBU.getObjects(codigo.Text);
  For i := l.Count downto 1 do Begin
    objeto := TTUnidadesNBU(l.Items[i-1]);
    inc(j);
    U.Cells[0, j] := objeto.Codigo;
    U.Cells[1, j] := objeto.Periodo;
    U.Cells[2, j] := FloatToStr(objeto.Unidades);
    U.Cells[3, j] := FloatToStr(objeto.Unidaddif);
  end;
  U.Row := 1;
  l.Destroy; l := Nil;
end;
...
begin
  if nbu.Buscar(codigo.Text) then
   if utiles.BajaRegistro(' Seguro que desea Eliminar Determinación ' + descrip.Text + ' ?') then Begin
     ...
     unidadesnbu.Borrar(codigo.Text);
...
begin
  ...
  U.Cells[0, 0] := 'Código'; U.Cells[1, 0] := 'Período';
  U.Cells[2, 0] := 'Unidad'; U.Cells[3, 0] := 'Unidad Dif.';
...
procedure TfmTablaNBU.periodoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then ActiveControl := unidades;
  if (Key = VK_RETURN) or (Key = VK_DOWN) then
    if (utiles.verificarPeriodo(periodo.Text)) then
      codanalisis.setFocus;
end;

procedure TfmTablaNBU.unidadesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then ActiveControl := unidad;
  if (Key = VK_RETURN) or (Key = VK_DOWN) then
    if Length(Trim(unidades.Text)) >= 0 then Begin
  ...
  if (Key = VK_RETURN) or (Key = VK_DOWN) then Begin
    if Length(Trim(codanalisis.Text)) > 0 then Begin
      if nomeclatura.Buscar(codanalisis.Text) then Begin
        CargarDatosAnalisis;
        nbu.Grabar(codigo.Text, descrip.Text, codanalisis.Text, StrToFloat(unidad.Text), StrToFloat(unidades.Text));
        CargarCodigos;
        unidadesnbu.Registrar(codigo.Text, periodo.Text, StrToFloat(unidad.Text), StrToFloat(unidades.Text));
        CargarUnidades;
        codanalisis.Text := ''; des.Caption := ''; unidad.Text := ''; unidades.Text := '';
...
procedure TfmTablaNBU.UKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (length(trim(U.Cells[0, U.Row])) > 0) then begin
    if (utiles.msgSiNo('Seguro para Borrar Código ' + U.Cells[0, U.Row] + ' en Período ' + U.Cells[1, U.Row] + ' ?')) then begin
      unidadesNBU.Borrar(U.Cells[0, U.Row], U.Cells[1, U.Row]);
      CargarUnidades;
    end;
  end else
    utiles.msgError('El Registro Seleccionado es Incorrecto ...!');
