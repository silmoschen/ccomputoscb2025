...
var
  fmEleMedicos: TfmEleMedicos;

implementation

uses CMedicosCCB, CUtiles, CUtilidadesStringGrid, CConfigForms;

{$R *.DFM}

procedure TfmEleMedicos.SeleccionarProf(xidprof: String);
var
  i: integer;
begin
...
procedure TfmEleMedicos.FijarProfPredeterminado(xidprof: String);
begin
  lista := TStringList.Create;
  lista.Add(xidprof);
end;

procedure TfmEleMedicos.CargarDatos;
var
  r: TQuery; i: integer;
begin
...
procedure TfmEleMedicos.SiClick(Sender: TObject);
begin
...
procedure TfmEleMedicos.TodosClick(Sender: TObject);
var
  i: integer;
begin
...
procedure TfmEleMedicos.NingunoClick(Sender: TObject);
var
  i: integer;
begin
...
procedure TfmEleMedicos.PClick(Sender: TObject);
begin
...
procedure TfmEleMedicos.buscarChange(Sender: TObject);
var
...
procedure TfmEleMedicos.buscarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmEleMedicos.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i, j: Integer;
begin
  configform.Guardar(fmEleMedicos, redim);
  lista := TStringList.Create;
  lista.Clear;
...
procedure TfmEleMedicos.FKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if Key = VK_ESCAPE then Close;
...
procedure TfmEleMedicos.ActualizarClick(Sender: TObject);
begin
  CargarDatos;
end;

procedure TfmEleMedicos.FormCreate(Sender: TObject);
begin
  lista := TStringList.Create;
end;

procedure TfmEleMedicos.FormShow(Sender: TObject);
var
  i: Integer;
begin
  if not configform.Setear(fmEleMedicos) then Left := StrToInt(FormatFloat('####', (Screen.DesktopWidth / 2) - (Width / 2)));
  buscar.Text   := '';
...
procedure TfmEleMedicos.Panel3Resize(Sender: TObject);
begin
  redim := True;
end;
