...
TTPadronOS = class
  Nrodoc, Codos, Nombre, Observacion, Fechanac, Retiva: String;
  tabla: TTable;
 public
  ...
  procedure   Guardar(xcodos, xnrodoc, xnombre, xretiva: String); overload;
...
procedure TTPadronOS.Guardar(xcodos, xnrodoc, xnombre, xretiva: String);
...
Begin
  if Buscar(xcodos, xnrodoc) then Begin
    ...
    Codos := ''; Nrodoc := ''; Nombre := '*** INEXISTENTE ***'; observacion := ''; fechanac := ''; retiva := '';
  end;
end;
...
Begin
  Result := TrimRight(tabla.FieldByName('nrodoc').AsString);
end;
...
Begin
  if (tabla <> Nil) then Result := tabla.RecordCount;
end;

function TTPadronOS.conectar(xcodos: String): Boolean;
// Objetivo...: cerrar tablas de persistencia
begin
  tabla := datosdb.openDB('bioqafil', '', '', auditoriacb.cabauditoria.DatabaseName);
  tabla.Open;
  idanter := xcodos;
