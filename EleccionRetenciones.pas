...
type
  ...
  public
    { Public declarations }
    tipo, IdSel, NomSel: string; intro, redim, ProfSeleccionados: Boolean; cantidadSel: Integer;
    lista: TStringList;
...
var
  fmEleRetenciones: TfmEleRetenciones;

implementation

uses CRetencionesCentroBioq, CUtiles, CUtilidadesStringGrid, CConfigForms;

{$R *.DFM}

procedure TfmEleRetenciones.SeleccionarProf(xidprof: String);
var
  i: integer;
begin
...
procedure TfmEleRetenciones.FijarProfPredeterminado(xidprof: String);
begin
  lista := TStringList.Create;
  lista.Add(xidprof);
end;

procedure TfmEleRetenciones.CargarDatos;
var
  r: TQuery; i: integer;
begin
  ...
  while not r.EOF do Begin
    ...
    F.cells[1, i] := r.FieldByName('items').AsString;
    r.Next;
  end;
  r.Close; r.Free;
  nroItems := i;
...
procedure TfmEleRetenciones.SiClick(Sender: TObject);
begin
...
procedure TfmEleRetenciones.TodosClick(Sender: TObject);
var
  i: integer;
begin
...
procedure TfmEleRetenciones.NingunoClick(Sender: TObject);
var
  i: integer;
begin
...
procedure TfmEleRetenciones.PClick(Sender: TObject);
begin
...
procedure TfmEleRetenciones.buscarChange(Sender: TObject);
var
...
procedure TfmEleRetenciones.buscarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmEleRetenciones.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i, j: Integer;
begin
  configform.Guardar(fmEleRetenciones, redim);
  lista := TStringList.Create;
  lista.Clear;
...
procedure TfmEleRetenciones.FKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if Key = VK_ESCAPE then Close;
...
procedure TfmEleRetenciones.ActualizarClick(Sender: TObject);
begin
  CargarDatos;
end;

procedure TfmEleRetenciones.FormCreate(Sender: TObject);
begin
  lista := TStringList.Create;
end;

procedure TfmEleRetenciones.FormShow(Sender: TObject);
var
  i: Integer;
begin
  if not configform.Setear(fmEleRetenciones) then Left := StrToInt(FormatFloat('####', (Screen.DesktopWidth / 2) - (Width / 2)));
  buscar.Text   := '';
...
procedure TfmEleRetenciones.Panel3Resize(Sender: TObject);
begin
  redim := True;
end;
