...
function medicoscabol: TTMedicosOL;
...
constructor TTMedicosOL.Create;
begin
  inherited Create;
  ...
  tabla := datosdb.openDB('medicos_cabos', '', '', conexion);
end;

destructor TTMedicosOL.Destroy;
begin
  inherited Destroy;
end;

procedure TTMedicosOL.Grabar(xcodos, xIdprof, xNombre: string);
...
procedure TTMedicosOL.Borrar(xcodos, xIdprof: string);
...
function TTMedicosOL.Buscar(xcodos, xIdprof: string): boolean;
...
procedure  TTMedicosOL.getDatos(xcodos, xIdprof: string);
...
function TTMedicosOL.setMedicosAlf: TQuery;
...
function TTMedicosOL.Nuevo: string;
...
procedure TTMedicosOL.Listar(orden, iniciar, finalizar, ent_excl: string; salida: char);
...
procedure TTMedicosOL.ListLinea(salida: char);
begin
...
function TTMedicosOL.BuscarPorNombre(xexpr: string): Boolean;
...
procedure TTMedicosOL.BuscarPorId(xexpr: string);
begin
  if tabla.IndexFieldNames <> 'Idprof' then tabla.IndexFieldNames := 'Idprof';
  tabla.FindNearest([xexpr]);
end;

function TTMedicosOL.ExistenProfIgualNombre(xexpr: String): Boolean;
...
procedure   TTMedicosOL.Filtrar(xcodos: string);
begin
  datosdb.Filtrar(tabla, 'codos = ' + '''' + xcodos + '''');
end;

procedure   TTMedicosOL.QuitarFiltro;
begin
  datosdb.QuitarFiltro(tabla);
end;

function    TTMedicosOL.getMedicos(xcodos: string): TQuery;
begin
  result := datosdb.tranSQL(tabla.DataBaseName, 'select * from ' + tabla.TableName + ' where codos = ' + '''' + xcodos + '''' + ' order by nombre');
end;

procedure TTMedicosOL.conectar;
// Objetivo...: Abrir tablas de persistencia
begin
...
procedure TTMedicosOL.desconectar;
