...
type
  ...
  public
    { Public declarations }
    tipo, IdSel, NomSel: string; profSeleccionados, intro, filtrarProfFacturacion, redim: Boolean; cantidadSel: Integer;
    lista, listaemails: TStringList;
...
var
  fmEleProfesional: TfmEleProfesional;

implementation

uses CProfesionalCCB, CUtiles, CUtilidadesStringGrid, CConfigForms;

{$R *.DFM}

procedure TfmEleProfesional.SeleccionarProf(xidprof: String);
var
  i: integer;
begin
...
procedure TfmEleProfesional.FijarProfPredeterminado(xidprof: String);
begin
  lista := TStringList.Create;
  lista.Add(xidprof);
end;

procedure TfmEleProfesional.CargarDatos;
var
  r: TQuery; i: integer;
begin
  ...
  if todosprof.Checked then r := profesional.setProfesionalesAlf else r := profesional.setProfesionalesAlfRetIVA;
  r.Open; i := 0; r.First;
  while not r.EOF do Begin
    profesional.getDatos(r.FieldByName('idprof').AsString);
    if not filtrarProfFacturacion then Begin
      Inc(i);
...
procedure TfmEleProfesional.SiClick(Sender: TObject);
begin
...
procedure TfmEleProfesional.TodosClick(Sender: TObject);
var
  i: integer;
begin
...
procedure TfmEleProfesional.NingunoClick(Sender: TObject);
var
  i: integer;
begin
  For i := 1 to nroItems do F.cells[2, i] := ' ';
  lista := Nil; listaemails := Nil;
  buscar.Text := '';
  F.Refresh;
end;

procedure TfmEleProfesional.PClick(Sender: TObject);
begin
...
procedure TfmEleProfesional.buscarChange(Sender: TObject);
var
...
procedure TfmEleProfesional.buscarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmEleProfesional.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i, j: Integer;
begin
  configform.Guardar(fmEleProfesional, redim);
  lista := TStringList.Create; listaemails := TStringList.Create;
  lista.Clear;
  Label3.Caption := IntToStr(F.row);

  j := 0; ProfSeleccionados := False; cantidadSel := 0; __selectAll := true;
  For i := 1 to F.RowCount - 1 do Begin
    ...
    if Copy(F.cells[2, i], 1, 1) = 'S' then Begin
      ...
      if (Length(trim(F.Cells[3, i])) > 0) then listaemails.Add(F.Cells[3, i]);
    end;
    if Copy(F.cells[2, i], 1, 1) <> 'S' then __selectAll := false;
  end;
  cantidadSel := j;
  if j = 1 then
    if F.Row > 1 then F.Row := F.Row - 1;
...
procedure TfmEleProfesional.FKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if Key = VK_ESCAPE then Close;
...
procedure TfmEleProfesional.ActualizarClick(Sender: TObject);
begin
  CargarDatos;
end;

procedure TfmEleProfesional.FormCreate(Sender: TObject);
begin
  lista := TStringList.Create;
end;

procedure TfmEleProfesional.FormShow(Sender: TObject);
var
  i: Integer;
begin
  if not configform.Setear(fmEleProfesional) then Left := StrToInt(FormatFloat('####', (Screen.DesktopWidth / 2) - (Width / 2)));
  buscar.Text   := '';
...
procedure TfmEleProfesional.Panel3Resize(Sender: TObject);
begin
  redim := True;
end;

procedure TfmEleProfesional.todosprofClick(Sender: TObject);
begin
  CargarDatos;
end;

function TfmEleProfesional.selectAll: boolean;
begin
  result := __selectAll;
end;
