...
uses SysUtils, IBDatabase, IBCustomDataSet, IBTable, IBQuery,
     DB, Variants, IB, CUtiles, CBDT, Classes; //, CFirebirdConnect;
...
var
  i: integer;
Begin
   {if (fconn = nil) then fconn := TTFirebirdConnect.Create;

   fconn.getModulo(xmodulo);
   modulo := fconn.Modulo;
   host := fconn.Usuario;
   password := fconn.Password;
   dir_remoto := fconn.Dir_Remoto;
   dir_remoto1 := fconn.Dir_Remoto1;}
