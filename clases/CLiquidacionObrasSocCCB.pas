...
uses Windows, CObrasSocialesCCB, CFacturacionCCB, CProfesionalCCB, SysUtils, CListar, DB, DBTables, CBDT, CUtiles,
     ...
     CAjustesIndivCentroBioq, CBackup, CCoseguros, CAuditoriaCCB, CDebitosCreditosCentroBioq, IBQuery;
     {,CFirebird, IBQuery;}
...
TTDistribucionObrasSociales = class(TObject)
  ...
  exporta_web, __liqTodos: boolean; tipoinf_web, interbase: string;
  ...
  procedure   getDatos(xperiodo, xcodos, xperliq, xnroLiq: String);
  procedure   Borrar(xperiodo, xcodos, xperliq, xnroLiq: String);
  procedure   GuardarLiquidacion(xperiodo, xcodos, xperiodoLiq, xfecha, xnroLiq: String; ximportetot, xporcentaje: Real);
  procedure   BorrarLiquidacionProfesional(xperiodo, xidprof, xnroLiq: String);
  procedure   BorrarMovimientosPeriodoCompleto(xperiodo, xperliq, xnroliq: String);
  function    setLiquidaciones(xperiodo, xnroliq: string): TStringList;

  { Ajustes Individuales }
  procedure   GuardarAjustesIndividuales(xperiodo, xcodos, xidprof, xitems, xconcepto, xfecha, xprotegido, xperiodoLiq, xnroLiq: String; ximporte: real; xCantidadItems: Integer);
  procedure   AjustarNumeroDeItems(xperiodo, xcodos, xnroLiq: String; CantItems: Integer);
  function    setItemsAjustesIndividuales(xperiodo, xcodos, xnroLiq: String): TQuery;
  function    setItemsAjustesIndividualesIB(xperiodo, xcodos, xnroLiq: String): TQuery;
  function    setItemsAjustesIndividualesProfesional(xperiodo, xidprof, xnroLiq: String): TQuery;
  function    setItemsAjustesIndividualesProfesionalIB(xperiodo, xidprof, xnroLiq: String): TQuery;
  function    setListaAjustesIndividualesProfesional(xperiodo, xidprof, xnroLiq: String): TStringList;
  function    setListaAjustesIndividualesProfesionalRetiva(xperiodo, xidprof, xnroLiq: String): TStringList;
  function    setListaAjustesIndividualesProfesionalExcluirRetiva(xperiodo, xidprof, xnroLiq: String): TStringList;
  function    setItemsLiquidados(xperiodo, xnroLiq: String): TStringList;

  { Débitos/Créditos Profesionales }
  procedure   GuardarDCProf(xperiodo, xnroLiq, xidprof, xitems, xidajuste, xdescripaj: String; ximporteaj: Real; xCantidadItems: Integer; xtipomov: Char);
  procedure   BorrarDCProfFijos(xperiodo, xnroLiq: string);
  procedure   BorrarDCProfManuales(xperiodo, xnroLiq: string);
  procedure   BorrarDCProfExedentes(xperiodo, xnroLiq, xitems: String);
  function    setItemsDCProf(xperiodo, xnroLiq: String): TQuery; overload;
  function    setItemsDCProf(xperiodo, xidprof, xnroLiq: String): TQuery; overload;
  function    setItemsDCProfIB(xperiodo, xnroLiq: String): TQuery; overload;
  function    setItemsDCProfIB(xperiodo, xidprof, xnroLiq: String): TQuery; overload;
  function    setListaDCProf(xperiodo, xidprof, xnroLiq: String): TStringList;

  { Informes }
  procedure   InfDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; salida: Char; xobsocial_iva, xganancias, xrecmontos: Boolean; xretieneganancias, xnroLiq: String);
  procedure   ConfigurarInforme(xreporte: String; xcopias, xsalto: ShortInt);
  procedure   getConfiguracionInforme(xreporte: String);
  procedure   ListarObrasSocialesLiquidadas(xlista: TStringList; xperiodo, xnroliq: String; salida: Char);
  procedure   ListarResumenLisquidacion(xperiodo, xnroLiq: String; salida: char; xrecalcular_montos: Boolean);
  procedure   ListarResumenRetenciones(xperiodo, xnroLiq: String; xlista: TStringList; salida: char);
  procedure   RecalcularDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; xobsocial_iva, xganancias, xrecmontos: Boolean; xretieneganancias, xnroLiq: String);
  ...
  procedure   InicializarMontos(xperiodo, xnroLiq: String; xborrar_datos: Boolean);
  procedure   InicializarDepositos(xperiodo, xnroLiq: String);
  procedure   RegistrarDeposito(xperiodo, xidprof, xcodbco, xfecha, xobservacion, xnroLiq: String);
  procedure   BorrarDeposito(xperiodo, xidprof, xcodbco, xnroLiq: String);
  function    setDepositos(xperiodo, xnroLiq: String): TStringList;
  ...
  procedure   BorrarControlLiq(xperiodo, xperiodoliq, xcodos, xnroLiq: String);
 ...
 private
  ...
  totganancias, monto_g, __monto, __montoganancias: Real;
  unifica_ganancias, unif_cuit, noListar, initxt: Boolean;
  lista1, lista2, lista3, lista4, lote: TStringList;
  {ffirebird: TTFirebird;
  ressql: TIBQuery;}
  ressql: TQuery;

  procedure   AjustarNroItemsDCProf(xperiodo, xidprof: String; xnroitems: Integer; xtipomov: Char);
  procedure   ListDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; salida: Char; xobsocial_iva, xganancias: Boolean; xretieneganancias, xnroLiq: String);
  ...
  procedure   RegistrarTotal(xperiodo, xidprof, xitems, xnrodist: String; xmonto, xtotal: Real);
  procedure   ListarMontos(xfiltro, xperiodo, xnroLiq: String; salida: char);
  ...
  procedure   RegistrarMontoParaGanancias(xidprof, xnrodist, xperiodo: String; xmonto: Real);
  ...
  procedure   MarcarConsumoGanancias(xperiodo, xidprof, xnroliq, xconsumido: String);
  function    verificarConsumoGanancias(xperiodo, xidprof, xnroliq: String): Boolean;
end;
...
begin
  ...
  if (interbase = 'N') then controlLiq := datosdb.openDB('controlLiq', '', '', dbs.DirSistema + '\distribucion\control');
...
begin
  ...
  if (interbase = 'S') then begin
    ExisteLiquidacion := False;
    ressql := datosdb.tranSQL('select count(*) from cabajustesind where periodo = ' + '''' + xperiodo + '''' + ' and codos = ' + '''' + xcodos + '''' + ' and perliq = ' + '''' + xperliq + '''');
    ressql.Open;
    if (ressql.Fields[0].AsInteger > 0) then ExisteLiquidacion := true else ExisteLiquidacion := false;
...
begin
  ...
  if (interbase = 'S') then begin
    lote.Clear;
    lote.add('delete from cabajustesind where periodo = ' + QuotedStr(xperiodo) + ' and codos = ' + QuotedStr(xcodos) + ' and perliq = ' + QuotedStr(xperiodoliq) + ' and nrodist = ' + QuotedStr(xnroLiq));
    lote.Add('insert into cabajustesind (periodo, codos, perliq, nrodist, importe, porcentaje, fecha) values (' +
             QuotedStr(xperiodo) + ', ' + QuotedStr(xcodos) + ', ' + QuotedStr(xperiodoLiq) + ', ' + QuotedStr(xnroLiq) + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(ximportetot), ',', '.'), 2, '0') + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(xporcentaje), ',', '.'), 2, '0') + ', ' +  QuotedStr(utiles.sExprFecha2000(xfecha)) + ')');
    ExisteLiquidacion := True;
    GuardarControlLiq(xperiodo, xperiodoliq, xcodos, xporcentaje);

    datosdb.TransacSQLBatch(lote);
    lote.clear;
  end;

end;

procedure TTDistribucionObrasSociales.GuardarAjustesIndividuales(xperiodo, xcodos, xidprof, xitems, xconcepto, xfecha, xprotegido, xperiodoLiq, xnroLiq: String; ximporte: real; xCantidadItems: Integer);
...
begin
  ...
  if (interbase = 'S') then begin
    if ximporte <> 0 then Begin
      lote.Add('delete from detajindiv where periodo = ' + QuotedStr(xperiodo) + ' and codos = ' + QuotedStr(xcodos) + ' and idprof = ' + QuotedStr(xidprof) +
               ' and items = ' + QuotedStr(xitems) + ' and nrodist = ' + QuotedStr(xnroLiq) + ' and perliq = ' + QuotedStr(xperiodoliq));

      lote.Add('insert into detajindiv (periodo, codos, idprof, items, nrodist, concepto, perliq, importe, protegido) values (' +
               QuotedStr(xperiodo) + ', ' + QuotedStr(xcodos) + ', ' + QuotedStr(xidprof) + ', ' + QuotedStr(xitems) + ', ' + QuotedStr(xnroLiq) + ', ' +
               QuotedStr(xconcepto) + ', ' + QuotedStr(xperiodoliq) + ', ' +
               utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(ximporte), ',', '.'), 2, '0') + ', ' +
               QuotedStr(xprotegido) + ')');
...
procedure TTDistribucionObrasSociales.AjustarNumeroDeItems(xperiodo, xcodos, xnroLiq: String; CantItems: Integer);
begin
  if (interbase = 'N') then begin
    ...
    detajust.close; detajust.Open;
  end;
  if (interbase = 'S') then begin
    lote.Add('DELETE FROM detajindiv WHERE periodo = ' + '"' + xperiodo + '"' + ' and codos = ' + '"' + xcodos + '"' + ' and nrodist = ' + '"' + xnroLiq + '"' + ' and items >= ' + '"' + utiles.sLlenarIzquierda(IntToStr(cantitems), 3, '0') + '"');
    datosdb.TransacSQLBatch(lote);
  end;
end;

procedure TTDistribucionObrasSociales.getDatos(xperiodo, xcodos, xperliq, xnroLiq: String);
...
begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select * from cabajustesind where periodo = ' + QuotedStr(xperiodo) + ' and codos = ' + QuotedStr(xcodos) +
                                      ' and perliq = ' + QuotedStr(xperliq) + ' and nrodist = ' + QuotedStr(xnroLiq));
    ressql.Open;
    if (ressql.RecordCount > 0) then begin
      Periodo    := xperiodo;
      Codos      := xcodos;
      fecha      := utiles.sFormatoFecha(ressql.FieldByName('Fecha').AsString);
      importe    := ressql.FieldByName('importe').AsFloat;
      periodoLiq := ressql.FieldByName('perLiq').AsString;
      porcentaje := ressql.FieldByName('porcentaje').AsFloat;
    end else Begin
      periodo := ''; codos := ''; importe := 0; porcentaje := 0; fecha := utiles.setFechaActual; periodoLiq := '';
    end;
    ressql.Close; ressql.Free;
...
procedure TTDistribucionObrasSociales.Borrar(xperiodo, xcodos, xperliq, xnroLiq: String);
...
begin
  if (interbase = 'N') then begin
    if Buscar(xperiodo, xcodos, xperliq) then Begin
      cabajust.Delete;
      datosdb.refrescar(cabajust);
      BorrarControlLiq(xperiodo, xperliq, xcodos, xnroLiq);
    end;
  end;
  if (interbase = 'S') then begin
    lote.Clear;
    lote.Add('delete from cabajustind where periodo = ' + QuotedStr(xperiodo) + ' and perliq = ' + QuotedStr(xperliq) + ' and codos = ' + QuotedStr(xnroLiq));
    BorrarControlLiq(xperiodo, xperliq, xcodos, xnroLiq);
    datosdb.TransacSQLBatch(lote);
  end;
end;

procedure TTDistribucionObrasSociales.BorrarLiquidacionProfesional(xperiodo, xidprof, xnroLiq: String);
...
begin
  ...
  if (interbase = 'N') then begin
    lote.Clear;
    lote.add('delete from detajindiv WHERE perliq = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' and nrodist = ' + '"' + xnroliq + '"');
    lote.add('delete from dcprofes WHERE perliq = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' and nrodist = ' + '"' + xnroliq + '"');
    datosdb.TransacSQLBatch(lote);
    lote.clear;
  end;
end;

procedure TTDistribucionObrasSociales.BorrarMovimientosPeriodoCompleto(xperiodo, xperliq, xnroliq: String);
// Objetivo...: Borrar Movimientos de una Obra Social
begin
  if (interbase = 'N') then begin
  ...
  if (interbase = 'S') then begin
    lote.clear;
    lote.add('DELETE FROM detajindiv WHERE periodo = ' + '"' + xperiodo + '"' + ' AND nrodist = ' + '"' + xnroLiq + '"');
    lote.add('DELETE FROM cabajindiv WHERE periodo = ' + '"' + xperiodo + '"' + ' AND nrodist = ' + '"' + xnroLiq + '"');
    lote.add('DELETE FROM dcprofes WHERE periodo = ' + '"' + xperliq + '"' + ' AND nrodist = ' + '"' + xnroLiq + '"');
    lote.add('DELETE FROM detajindiv WHERE periodo = ' + '"' + xperliq + '"' + ' AND nrodist = ' + '"' + xnroLiq + '"');
    datosdb.TransacSQLBatch(lote);
    lote.Clear;
  end;
end;

function TTDistribucionObrasSociales.setLiquidaciones(xperiodo, xnroliq: string): TStringList;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select * from cabajustesind where periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroliq + '''');
    ressql.Open;
    l := TStringList.Create;
    while not ressql.Eof do Begin
      l.Add(ressql.FieldByName('codos').AsString + ressql.FieldByName('perliq').AsString + ressql.FieldByName('fecha').AsString + utiles.FormatearNumero(ressql.FieldByName('importe').AsString) + ';1' + utiles.FormatearNumero(ressql.FieldByName('porcentaje').AsString));
      ressql.Next;
    end;
    ressql.close; ressql.free;
    Result := l;
...
function TTDistribucionObrasSociales.setItemsAjustesIndividuales(xperiodo, xcodos, xnroLiq: String): TQuery;
// Objetivo...: Devolver ajustes de una obra social
begin
  if (interbase = 'N') then Result := datosdb.tranSQL(directorio, 'SELECT * FROM ' + detajust.TableName + ' WHERE periodo = ' + '"' + xperiodo + '"' + ' AND codos = ' + '"' + xcodos + '"');
end;

function TTDistribucionObrasSociales.setItemsAjustesIndividualesIB(xperiodo, xcodos, xnroLiq: String): TQuery;
// Objetivo...: Devolver ajustes de una obra social
begin
  if (interbase = 'S') then Result := datosdb.getTransacSQL('SELECT * FROM detajindiv WHERE periodo = ' + '"' + xperiodo + '"' + ' AND codos = ' + '"' + xcodos + '"' + ' and nrodist = ' + '"' + xnroliq + '"');
end;

function TTDistribucionObrasSociales.setItemsAjustesIndividualesProfesional(xperiodo, xidprof, xnroliq: String): TQuery;
// Objetivo...: Devolver Ajustes de un Profesional
begin
  if (interbase = 'N') then Result := datosdb.tranSQL(directorio, 'SELECT * FROM ' + detajust.TableName + ' WHERE perliq = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"');
  datosdb.refrescar(detajust);
end;

function TTDistribucionObrasSociales.setItemsAjustesIndividualesProfesionalIB(xperiodo, xidprof, xnroliq: String): TQuery;
// Objetivo...: Devolver Ajustes de un Profesional
begin
  if (interbase = 'S') then Result := datosdb.getTransacSQL('SELECT * FROM detajindiv WHERE perliq = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND nrodist = ' + '"' + xnroliq + '"');
end;

function TTDistribucionObrasSociales.setListaAjustesIndividualesProfesional(xperiodo, xidprof, xnroliq: String): TStringList;
// Objetivo...: Devolver Ajustes de un Profesional
var
  l: TStringList;
  e: Boolean;
begin
  ...
  if (interbase = 'S') then begin
    //utiles.msgError('select * from detajindiv perliq = ' + '''' + xperiodo + '''' + ' AND idprof = ' + '''' + xidprof + '''' + ' and nrodist = ' + '"' + xnroliq + '"');
    ressql := datosdb.getTransacSQL('select * from detajindiv where perliq = ' + '''' + xperiodo + '''' + ' AND idprof = ' + '''' + xidprof + '''' + ' and nrodist = ' + '"' + xnroliq + '"');
    ressql.Open;
    while not ressql.Eof do Begin
      l.Add(ressql.FieldByName('periodo').AsString + ressql.FieldByName('codos').AsString + ressql.FieldByName('idprof').AsString + ressql.FieldByName('items').AsString + ressql.FieldByName('perliq').AsString + ressql.FieldByName('concepto').AsString + ';1' + ressql.FieldByName('importe').AsString);
      ressql.Next;
    end;
    ressql.Close; ressql.Free;
    Result := l;
...
function TTDistribucionObrasSociales.setListaAjustesIndividualesProfesionalRetiva(xperiodo, xidprof, xnroliq: String): TStringList;
// Objetivo...: Devolver Ajustes de un Profesional
var
  l: TStringList;
  e: Boolean;
begin
  ...
  if (interbase = 'S') then begin
    ...
    while not ressql.Eof do Begin
      ...
      if obsocial.Retieneiva = 'S' then l.Add(ressql.FieldByName('periodo').AsString + ressql.FieldByName('codos').AsString + ressql.FieldByName('idprof').AsString + ressql.FieldByName('items').AsString + ressql.FieldByName('perliq').AsString + ressql.FieldByName('concepto').AsString + ';1' + ressql.FieldByName('importe').AsString);
      ressql.Next;
    end;
    ressql.close; ressql.free;
    Result := l;
...
function TTDistribucionObrasSociales.setListaAjustesIndividualesProfesionalExcluirRetiva(xperiodo, xidprof, xnroliq: String): TStringList;
// Objetivo...: Devolver Ajustes de un Profesional
var
  l: TStringList;
  e: Boolean;
begin
  ...
  if (interbase = 'S') then begin
    ...
    while not ressql.Eof do Begin
      obsocial.getDatos(ressql.FieldByName('codos').AsString);
      if obsocial.Retieneiva = 'N' then l.Add(ressql.FieldByName('periodo').AsString + ressql.FieldByName('codos').AsString + ressql.FieldByName('idprof').AsString + ressql.FieldByName('items').AsString + ressql.FieldByName('perliq').AsString + ressql.FieldByName('concepto').AsString + ';1' + ressql.FieldByName('importe').AsString);
      ressql.Next;
    end;
    ressql.close; ressql.free;
    Result := l;
...
procedure TTDistribucionObrasSociales.InicializarMontos(xperiodo, xnroLiq: String; xborrar_datos: Boolean);
...
begin
  ...
  if (interbase = 'S') then begin
    lote.Clear;
    if xborrar_datos then begin
      if (ExcluirObrasSocialesInscriptas = 2) then lote.Add('delete from montos where periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' + ' and items >= ' + '''' + 'S' + '''') else
        lote.Add('delete from montos where periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
      datosdb.TransacSQLBatch(lote);
    end;
  end;
end;

procedure TTDistribucionObrasSociales.InicializarDepositos(xperiodo, xnroLiq: String);
...
begin
  if (interbase = 'N') then Result := datosdb.Buscar(depositos, 'idprof', 'codbco', xidprof, xcodbco);
  if (interbase = 'S') then result := false;
end;

procedure TTDistribucionObrasSociales.RegistrarMontoParaGanancias(xidprof, xnrodist, xperiodo: String; xmonto: Real);
...
Begin
  ...
  if (interbase = 'S') then begin
    m := 0; m1 := 0;
    if xnrodist = '01' then m  := xmonto;
    if xnrodist = '02' then m1 := xmonto;
    lote.Clear;
    lote.Add('delete from subtotganancias where periodo = ' + QuotedStr(xperiodo) + ' and idprof = ' + QuotedStr(xidprof));
    lote.Add('insert into subtotganancias (periodo, idprof, monto, monto1) values (' +
             QuotedStr(xperiodo) + ', ' +
             QuotedStr(xnrodist) + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(m), ',', '.'), 2, '0') + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(m1), ',', '.'), 2, '0') + ')');
    datosdb.TransacSQLBatch(lote);
    lote.clear;
  end;
end;
...
Begin
  ...
  if (interbase = 'S') then begin
    monto := 0;
    ressql := datosdb.getTransacSQL('select * from subtotganancias where periodo = ' + QuotedStr(Copy(xperiodo, 1, 2) + Copy(xperiodo, 4, 4)) + ' and idprof = ' + QuotedStr(xidprof));
    ressql.Open;
    monto := ressql.FieldByName('monto').AsFloat;
    ressql.Close; ressql.Free;

    ressql := datosdb.getTransacSQL('select * from subtotganancias where periodo = ' + QuotedStr(Copy(xperiodo, 1, 2) + Copy(xperiodo, 4, 4)));
    ressql.Open;
    // Ahora Buscar los CUIT dependientes, para unificar el monto
    l := retenciones.setCuitProfesional(xidprof);
    For i := 1 to l.Count do Begin
      ...
      ressql.First;
      while not ressql.Eof do Begin
        if ressql.FieldByName('idprof').AsString = Copy(l.Strings[i-1], p+2, 6) then monto := monto + ressql.FieldByName('monto').AsFloat;
        ressql.Next;
      end;
    end;
...
procedure TTDistribucionObrasSociales.RegistrarDeposito(xperiodo, xidprof, xcodbco, xfecha, xobservacion, xnroLiq: String);
// Objetivo...: Registrar Depositos
begin
  if (interbase = 'N') then begin
    InicializarDepositos(xperiodo, xnroLiq);
    depositos.Open;
  ...
  if (interbase = 'S') then begin
    InicializarDepositos(xperiodo, xnroLiq);
    lote.clear;
    lote.add('delete from depositos where periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + xidprof + '''' + ' and codbco = ' + '''' + xcodbco + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
    lote.Add('insert into depositos(periodo, idprof, codbco, nrodist, fecha, observacion) values (' +
             QuotedStr(xperiodo) + ', ' + QuotedStr(xidprof) + ', ' + QuotedStr(xcodbco) + ', ' + QuotedStr(xnroLiq) + ', ' +
             QuotedStr(utiles.sExprFecha2000(xfecha)) + ', ' + QuotedStr(xobservacion) + ')');
    datosdb.TransacSQLBatch(lote);
    lote.clear;
  end;
end;

procedure TTDistribucionObrasSociales.BorrarDeposito(xperiodo, xidprof, xcodbco, xnroLiq: String);
...
function  TTDistribucionObrasSociales.setDepositos(xperiodo, xnroliq: String): TStringList;
// Objetivo...: Inicializar Depositos
var
  l: TStringList;
begin
  ...
  if (interbase = 'S') then begin
    l := TStringList.Create;
    ressql := datosdb.getTransacSQL('select * from depositos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' +xnroliq + '''');
    ressql.Open;
    while not ressql.Eof do Begin
      l.Add(ressql.FieldByName('idprof').AsString + ressql.FieldByName('codbco').AsString + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString) + ressql.FieldByName('periodo').AsString + ressql.FieldByName('observacion').AsString);
      ressql.Next;
    end;
    ressql.close; ressql.Free;
    Result := l;
...
function TTDistribucionObrasSociales.setItemsLiquidados(xperiodo, xnroliq: String): TStringList;
// Objetivo...: Devolver Ajustes de un Periodo
var
  l: TStringList;
  t: Boolean;
begin
  ...
  if (interbase = 'S') then begin
    ...
    ressql.Open;
    while not ressql.Eof do Begin
      l.Add(ressql.FieldByName('periodo').AsString + ressql.FieldByName('codos').AsString + ressql.FieldByName('perliq').AsString + ressql.FieldByName('importe').AsString + ';1' + ressql.FieldByName('porcentaje').AsString + ';2' + ressql.FieldByName('fecha').AsString);
      ressql.Next;
    end;
    ressql.close; ressql.free;
    Result := l;
...
procedure TTDistribucionObrasSociales.GuardarDCProf(xperiodo, xnroLiq, xidprof, xitems, xidajuste, xdescripaj: String; ximporteaj: Real; xCantidadItems: Integer; xtipomov: Char);
begin
  if (interbase = 'N') then begin
    ...
    if xCantidadItems > 0 then
      if StrToInt(xitems) = xCantidadItems then begin
        AjustarNroItemsDCProf(xperiodo, xidprof, xCantidadItems, xtipomov);
        dcprof.Close;
        dcprof.Open;
      end;
  end;
  ...
  if (interbase = 'S') then begin
    lote.Add('insert into dcprofes (periodo, nrodist, idprof, items, idajuste, descripaj, importeaj, tipomov) values (' +
             QuotedStr(xperiodo) + ', ' + QuotedStr(xnroLiq) + ', ' + QuotedStr(xidprof) + ', ' + QuotedStr(xitems) + ', ' + QuotedStr(xidajuste) + ', ' +
             QuotedStr(xdescripaj) + ', ' + utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(ximporteaj), ',', '.'), 2, '0') + ', ' +
             QuotedStr(xtipomov) + ')');
  end;
end;

procedure TTDistribucionObrasSociales.BorrarDCProfFijos(xperiodo, xnroLiq: string);
// Objetivo...: Borrar Items Manuales
Begin
  if (interbase = 'N') then datosdb.tranSQL(directorio, 'delete from ' + dcprof.TableName + ' where tipomov = ' + '"' + 'F' + '"');
  if (interbase = 'S') then datosdb.tranSQL('delete from dcprofes where tipomov = ' + '"' + 'F' + '"' + ' and periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroliq + '''');
end;

procedure TTDistribucionObrasSociales.BorrarDCProfManuales(xperiodo, xnroliq: string);
// Objetivo...: Borrar Items Manuales
Begin
  if (interbase = 'N') then datosdb.tranSQL(directorio, 'delete from ' + dcprof.TableName + ' where tipomov = ' + '"' + 'M' + '"');
  if (interbase = 'S') then begin
    lote.Clear;
    lote.Add('delete from dcprofes where tipomov = ' + '"' + 'M' + '"' + ' and periodo = ' + '"' + xperiodo + '"' + ' and nrodist = '  + '"' + xnroLiq + '"');
  end;
end;

procedure TTDistribucionObrasSociales.BorrarDCProfExedentes(xperiodo, xnroLiq, xitems: String);
// Objetivo...: Borrar Exedentes
Begin
  if (interbase = 'N') then datosdb.tranSQL(directorio, 'delete from ' + dcprof.TableName + ' where items > ' + '"' + xitems + '"');
  if (interbase = 'S') then begin
    lote.Clear;
    lote.Add('delete from dcprofes where items > ' + '"' + xitems + '"' + ' and periodo = ' + '"' + xperiodo + '"' + ' and nrodist = '  + '"' + xnroLiq + '"');
  end;
end;
...
function  TTDistribucionObrasSociales.setItemsDCProf(xperiodo, xnroLiq: String): TQuery;
begin
  Result := datosdb.tranSQL(directorio, 'SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' ORDER BY Tipomov, Idprof, Items');
end;

function  TTDistribucionObrasSociales.setItemsDCProfIB(xperiodo, xnroLiq: String): TQuery;
begin
  Result := datosdb.getTransacSQL('SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' AND nrodist = ' + '''' + xnroliq + '''' + ' ORDER BY Tipomov, Idprof, Items');
end;

function TTDistribucionObrasSociales.setListaDCProf(xperiodo, xidprof, xnroliq: String): TStringList;
var
  l: TStringList;
  e: Boolean;
Begin
  ...
  if (interbase = 'S') then begin
    debitoscreditos.conectar;
    ressql := datosdb.getTransacSQL('select * from dcprofes where idprof = ' + '''' + xidprof + '''' + ' and periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroliq + '''' + ' and importeaj <> 0');  // importe <> 0 10/03/2015
    ressql.Open;
    //utiles.msgError('select * from dcprofes where idprof = ' + '''' + xidprof + '''' + ' and periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroliq + '''');
    while not ressql.Eof do Begin
      l.Add(ressql.FieldByName('periodo').AsString + ressql.FieldByName('idprof').AsString + ressql.FieldByName('items').AsString + ressql.FieldByName('descripaj').AsString + ';1' + ressql.FieldByName('importeaj').AsString + ';2' + ressql.FieldByName('tipomov').AsString);
      debitoscreditos.getDatos(ressql.FieldByName('idajuste').AsString);
      if (ressql.FieldByName('idajuste').AsString = '') or (ressql.FieldByName('tipomov').AsString = 'M') then
        lista4.Add('S')
      else
        lista4.Add(debitoscreditos.Liqsepa);
      ressql.Next;
...
function  TTDistribucionObrasSociales.setItemsDCProf(xperiodo, xidprof, xnroLiq: String): TQuery;
begin
  Result := datosdb.tranSQL(directorio, 'SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' ORDER BY Items');
end;

function  TTDistribucionObrasSociales.setItemsDCProfIB(xperiodo, xidprof, xnroLiq: String): TQuery;
begin
  Result := datosdb.getTransacSQL('SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND nrodist = ' + '"' + xnroliq + '"'  + ' AND tipomov = ' + '"M"' + ' ORDER BY Idprof, Items');
end;

{*******************************************************************************}

procedure TTDistribucionObrasSociales.RecalcularDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; xobsocial_iva, xganancias, xrecmontos: Boolean; xretieneganancias, xnroLiq: String);
// Objetivo...: Recalcular Montos Distribucion a Profesionales
Begin
  noListar := True;
  InfDistribucionProfesional(xperiodo, xperiodoLiq, listSel, listDist, 'P', xobsocial_iva, xganancias, xrecmontos, xretieneganancias, xnroLiq);
  noListar := False;
end;

procedure TTDistribucionObrasSociales.InfDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; salida: Char; xobsocial_iva, xganancias, xrecmontos: Boolean; xretieneganancias, xnroLiq: String);
...
Begin
  ...
  PrepararDirectorio(xperiodo);
  InicializarMontos(xperiodo, xperiodoLiq, xrecmontos);
  ...
  InicializarDepositos(xperiodo, xperiodoLiq);
  initxt := false;
  ...
  For i := 1 to listSel.Count do Begin
    ...
    if (l.Count = 0) or not (xganancias) then Begin        // Significa que No estan Unificados los CUIT
      ...
      s.Add(listSel[i-1]);
      ListDistribucionProfesional(xperiodo, xperiodoLiq, s, listDist, salida, xobsocial_iva, False, xretieneganancias, xnroliq);
      s.Destroy;
      ...
      For j := 1 to l.Count do Begin     // Acumulamos los totales en los CUIT Unificados
        ...
        ListDistribucionProfesional(xperiodo, xperiodoLiq, s, listDist, 'N', xobsocial_iva, xganancias, xretieneganancias, xnroliq);
        s.Destroy;
        unif_cuit  := False;
      end;
      //utiles.msgError(floattostr(monto_g));

      // 21/05/2020 - Retenemos el Monto Bruto del otro CUIT
      __montoganancias := monto_g;

      // Ahora largamos la Liquidación, con Ganancias Acumuladas
      s := TStringList.Create;
      s.Add(listSel[i-1]);
      ListDistribucionProfesional(xperiodo, xperiodoLiq, s, listDist, salida, xobsocial_iva, xganancias, xretieneganancias, xnroliq);
      s.Destroy;
...
procedure TTDistribucionObrasSociales.ListDistribucionProfesional(xperiodo, xperiodoLiq: String; listSel, listDist: TStringList; salida: Char; xobsocial_iva, xganancias: Boolean; xretieneganancias, xnroLiq: String);
...
var
  ...
  peranter, formula_ganancias, __c: String;
  rcos, rtcos: TQuery;
  ...
  begin
    ...
    if (interbase = 'S') then begin
      ressql := datosdb.getTransacSQL('select * from depositos where periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + xidprof + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
      ressql.Open;
      while not ressql.Eof do Begin
        entbcos.getDatos(ressql.FieldByName('codbco').AsString);
        if (salida = 'P') or (salida = 'I') then
          list.Linea(0, 0, '     Acreditado en  ' + entbcos.descrip + ', fecha  ' + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString), 1, 'Arial, cursiva, 8', salida, 'S');
        if (salida = 'T') then
          list.LineaTxt(CHR(15) + 'Acreditado en  ' + entbcos.descrip + ', fecha  ' + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString) + CHR(18), True);
        if (length(trim(ressql.FieldByName('observacion').AsString)) > 0) then begin
           if (salida = 'P') or (salida = 'I') then
             list.Linea(0, 0, ressql.FieldByName('observacion').AsString, 1, 'Arial, negrita, 8', salida, 'S');
           if (salida = 'T') then
             list.LineaTxt(CHR(15) + ressql.FieldByName('observacion').AsString + CHR(18), True);
        end;
...
Begin
  ...
  if listSel = Nil then utiles.msgError('No hay Profesional/Laboratorio Seleccionado ...!') else Begin
    ...
    if (interbase = 'S') then begin
      datosdb.TranSQL('delete from montosret where idprof = ' + '''' + listSel[i] + '''' + ' and periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
    end;
    ...
    For i := 0 to listSel.Count - 1 do Begin
     ...
     lista1 := setItemsLiquidados(Copy(xperiodo, 1, 7), xnroLiq);
     l := True;
       ...
       For ix := 1 to lista2.Count do Begin
         ...
         if Copy(lista1.Strings[it-1], 8, 6) = Copy(lista2.Strings[ix-1], 14, 6) then Begin
           ...
           if listob then Begin
             ...
             if (interbase = 'S') then begin
               // 05/08/2014 - Montos para la retencion del centro
               //utiles.msgError('3');
               lote.Clear;
               lote.Add('delete from montosret where periodo = ' + '''' + Copy(lista1.Strings[it-1], 14, 7) + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and codos = ' + '''' + Copy(lista2.Strings[ix-1], 14, 6) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
               lote.Add('insert into montosret (periodo, idprof, codos, nrodist, monto) values (' +
                        QuotedStr(Copy(lista1.Strings[it-1], 14, 7)) + ', ' +
                        QuotedStr(listSel[i]) + ', ' +
                        QuotedStr(Copy(lista2.Strings[ix-1], 14, 6)) + ', ' +
                        QuotedStr(xnroLiq) + ', ' +
                        utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres( Trim(Copy(lista2.Strings[ix-1], 20, 15)) , ',', '.'), 2, '0') + ')');
               datosdb.TransacSQLBatch(lote);
               lote.Clear;
             end;
     ...
     if l then Begin
       ...
       if not unif_cuit then RegistrarTotal(xperiodo, listSel[i], 'AAA', NroLiq, totales[1], 0);
     ...
     For it := 1 to lista1.Count do Begin
       ...
       For ix := 1 to lista2.Count do Begin
         ...
         if utiles.verificarItemsLista(plq, Copy(lista1.Strings[it-1], 14, 7) + Copy(lista1.Strings[it-1], 8, 6)) then Begin
           getDatos(Copy(xperiodo, 1, 7), Copy(lista1.Strings[it-1], 8, 6), Copy(lista1.Strings[it-1], 14, 7), xnroliq);
     ...
     if l then Begin
      ...
      if totales[1] < 0 then list.Linea(0, 0, ' ', 1, 'Arial, normal, 8', salida, 'S') else Begin
        ...
        if ExcluirObrasSocialesInscriptas = 0 then Begin
          lista2 := setListaAjustesIndividualesProfesional(Copy(xperiodo, 1, 7), listSel[i], xnroLiq);
        end else Begin
          profesional.SincronizarListaRetIVA(xperiodo, listSel[i]);

          if ExcluirObrasSocialesInscriptas = 2 then lista2 := setListaAjustesIndividualesProfesionalRetiva(Copy(xperiodo, 1, 7), listSel[i], xnroLiq) else
            if profesional.RetieneIva <> 'S' then lista2 := setListaAjustesIndividualesProfesional(Copy(xperiodo, 1, 7), listSel[i], xnroLiq) else
              lista2 := setListaAjustesIndividualesProfesionalExcluirRetiva(Copy(xperiodo, 1, 7), listSel[i], xnroLiq);
        end;
        ...
        For ix := 1 to lista2.Count do Begin
          ...
          if (interbase = 'S') then begin
             ...
             //utiles.msgError('4');
             __monto := 0;
             ressql := datosdb.getTransacSQL('select retencion from montosret where periodo = ' + '''' + Copy(lista1.Strings[ix-1], 14, 7) + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and codos = ' + '''' + Copy(lista2.Strings[ix-1], 14, 6) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
             ressql.open;
             if (ressql.RecordCount > 0) then __monto := ressql.Fields[0].AsFloat;
             ressql.close; ressql.free;

             lote.Clear;
             lote.Add('delete from montosret where periodo = ' + '''' + Copy(lista1.Strings[ix-1], 14, 7) + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and codos = ' + '''' + Copy(lista2.Strings[ix-1], 14, 6) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
             lote.Add('insert into montosret (periodo, idprof, codos, nrodist, retencion) values (' +
                        QuotedStr(Copy(lista1.Strings[ix-1], 14, 7)) + ', ' +
                        QuotedStr(listSel[i]) + ', ' +
                        QuotedStr(Copy(lista2.Strings[ix-1], 14, 6)) + ', ' +
                        QuotedStr(xnroLiq) + ', ' +
                        utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(__monto + StrToFloat(Trim(Copy(lista2.Strings[ix-1], p1 + 2, 15))) ), ',', '.'), 1, '0') + ')');
        ...
        if not unif_cuit then RegistrarTotal(xperiodo, listSel[i], 'BBB', NroLiq, totfinal, 0);
        ...
        if ExcluirObrasSocialesInscriptas <> 2 then lista2 := setListaDCProf(xperiodo, listSel[i], xnroliq) else lista2.Clear;
        ...
        For ix := 1 to lista2.Count do Begin
          ...
          if (utiles.verificarItemsLista(listdist, Copy(xperiodo, 8, 2))) or (Copy(lista2.Strings[ix-1], p2 + 2, 1) = 'M') then Begin    // Determina las Distribuciones donde se debe aplicar
            ...
            if ((LiquidarRetencionesSeparadas) and (lista4.Strings[ix-1] = 'S')) and not (unif_cuit) then begin
              ...
              //if not unif_cuit then utiles.msgError(Copy(lista2.Strings[ix-1], 14, 3) + '  ' + Copy(lista2.Strings[ix-1], 17, p1 - 17) + '   ' + Trim(Copy(lista2.Strings[ix-1], p1+2, (p2-p1)-2)));
              if not unif_cuit then RegistrarTotal(xperiodo, listSel[i], Copy(lista2.Strings[ix-1], 14, 3), NroLiq, StrToFloat(Trim(Copy(lista2.Strings[ix-1], p1+2, (p2-p1)-2))), 0);
            end;
        ...
        if ExcluirObrasSocialesInscriptas = 2 then lista3 := retenciones.setListaItemsRetienenIVA else
          lista3 :=  retenciones.setListaItems(xnroLiq); // 18/09/2015 retenciones.setListaItems;
              ...
              if (salida = 'P') or (salida = 'I') or (salida = 'T') then Begin
                ...
                if StrToFloat(Trim(Copy(lista3.Strings[ix-1], p2 + 2, (p3 - (p2+2))))) = 0 then Begin
                  ...
                  if not retenciones.setItemsDiferencial(Copy(lista3.Strings[ix-1], 1, 2)) then Begin
                    ...
                    // 20/05/2020 - Excluye Ins. Becario de Liquidaciones utiles.msgError(listSel[i] + ' ' + Copy(lista3.Strings[ix-1], 1, 2));

                    if (retenciones.getExcluye(Copy(lista3.Strings[ix-1], 1, 2), listSel[i])) then retenciones.PorcentajeTot := 0;
                    ...
                    if not unif_cuit then begin
                      if (ExcluirObrasSocialesInscriptas <> 2) then RegistrarTotal(xperiodo, listSel[i], 'R' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, (totfinal * (StrToFloat(Trim(Copy(lista3.Strings[ix-1], p1 + 2, p2-(p1+2)))) * 0.01)), totfinal);
                      if (ExcluirObrasSocialesInscriptas = 2) then RegistrarTotal(xperiodo, listSel[i], 'S' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, (totfinal * (StrToFloat(Trim(Copy(lista3.Strings[ix-1], p1 + 2, p2-(p1+2)))) * 0.01)), totfinal);
                    end
                    ...
                    if (interbase = 'S') then begin
                      ressql := datosdb.getTransacSQL('select * from montosret where periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
                      ressql.Open;
                      while not ressql.eof do begin
                        if not retenciones.setPorcentajeDiferencial(ressql.FieldByName('codos').AsString) then tot[1] := tot[1] + ( ressql.FieldByName('monto').AsFloat + ressql.FieldByName('retencion').AsFloat ) else
                          tot[6] := tot[6] + ( ressql.FieldByName('monto').AsFloat * ( retenciones.setPorcentajeDif(ressql.FieldByName('codos').AsString) * 0.01) );
                    ...
                    if not unif_cuit then Begin
                      if ExcluirObrasSocialesInscriptas <> 2 then RegistrarTotal(xperiodo, listSel[i], 'R' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, (tot[2] + tot[3]), totfinal);
                      if ExcluirObrasSocialesInscriptas = 2 then RegistrarTotal(xperiodo, listSel[i], 'S' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, (tot[2] + tot[3]), totfinal);
                    end;
                ...
                if not (zzt) and (profesional.AjusteDC = 'S') then Begin
                  ...
                  if not unif_cuit then Begin
                    ...
                    if xretieneganancias = 'N' then Begin
                      if ExcluirObrasSocialesInscriptas <> 2 then RegistrarMontoParaGanancias(listSel[i], '01', xperiodo, totfinal);
                    end;
                    //utiles.msgError('57');
                  end;
                  ...
                  if AplicarMinimoGanancias = 1 then Begin
                    if Copy(xperiodo, 8, 2) = '01' then Begin    // Deducimos el periodo para ver si consumio el minimo consumo
                      ...
                      if verificarConsumoGanancias(xperiodo, listSel[i], peranter) then minimo_ganancias := 0;
                    end;
                  end;
                      ...
                      if not unifica_ganancias then Begin
                        if (totfinal - StrToFloat(Trim(Copy(lista3.Strings[ix-1], p2 + 2, (p3 - (p2+2))))) > 0) then Begin
                          ...
                          if xretieneganancias = 'S' then Begin    // Solo Aplicamos Ganancias si está estipulado en el Período
                            ...
                            if minimo_ganancias > 0 then MarcarConsumoGanancias(xperiodo, listSel[i], Copy(xperiodo, 8, 2), 'S') else
                              MarcarConsumoGanancias(xperiodo, listSel[i], Copy(xperiodo, 8, 2), 'N');
                            ...
                            if not unif_cuit then RegistrarTotal(xperiodo, listSel[i], 'R' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, monto_ganancias, totfinal);
                        ...
                        if xretieneganancias = 'S' then Begin
                          ...
                          if AplicarMinimoGanancias = 2 then Begin
                            if Copy(xperiodo, 8, 2) = '02' then Begin
                              peranter := Copy(xperiodo, 1, 7) + '01';
                              if verificarConsumoGanancias(xperiodo, listSel[i], peranter){Copy(xperiodo, 8, 2))} then minimo_ganancias := 0;
                            end;
                          end;
                          ...
                          // Marcamos el Período de Ganancias Pagado - 08/05/2007
                          if minimo_ganancias > 0 then MarcarConsumoGanancias(xperiodo, listSel[i], Copy(xperiodo, 8, 2), 'S') else
                            MarcarConsumoGanancias(xperiodo, listSel[i], Copy(xperiodo, 8, 2), '');
                          //utiles.msgError(listSel[i] + '  ' + Copy(xperiodo, 8, 2));
                          //--------------------------------------------------
                          ...
                          totales[7] := totales[7] + monto_ganancias;
                          if not unif_cuit then RegistrarTotal(xperiodo, listSel[i], 'R' + Copy(lista3.Strings[ix-1], 1, 2), NroLiq, monto_ganancias, totganancias);
                        end;
                      end;
                    end;
                  end;
                  ...
                  //==========================================================
                  // CUIT UNIFICADOS - 21/05/2020
                  if (unifica_ganancias) then totganancias := totganancias + __montoganancias;
                  //utiles.msgError(floattostr(monto_ganancias));
                  //==========================================================
        ...
        if (ExcluirObrasSocialesInscriptas = 1) then begin
          ...
          if (interbase = 'S') then begin
            ...
            ressql.open;
            while not ressql.eof do begin
              if (coseguro.verificarCoseguro(ressql.FieldByName('perliq').asstring, ressql.FieldByName('codos').asstring)) then begin
                lcosegurocodos.Add(ressql.FieldByName('codos').asstring);
                lcoseguroperiodo.Add(ressql.FieldByName('perliq').asstring);
              end;
              ressql.Next;
            end;
            ressql.Close; ressql.Free;
            ...
            if (coseguro.verificarCoseguro(lcoseguroperiodo.Strings[ncoss-1])) then begin
              ...
              while not rcos.Eof do begin
                ...
                if (salida = 'P') or (salida = 'I') then Begin
                  ...
                  RegistrarTotal(xperiodo, listSel[i], 'R04', NroLiq, montocos, 0);
                End;

                if (salida = 'T') then Begin
                  obsocial.getDatos(rcos.FieldByName('codos').AsString);

                  __c := 'Cos. Per. ' + lcoseguroperiodo.Strings[ncoss-1] + ' ' + rcos.FieldByName('codos').AsString + ' ' + copy(obsocial.Nombre,1, 25);
                  list.LineaTxt(CHR(15) + utiles.espacios(25) + __c + utiles.espacios(66 - Length(TrimRight(__c))), False);

                  list.importeTxt(montocos, 15, 2, True);
                  Inc(lineas); if controlarsalto then titulosIFD(salida);
        ...
        if (salida = 'P') or (salida = 'I') then Begin
          ...
          if not (LiquidarRetencionesSeparadas) then begin
            ...
            if (interbase = 'S') then begin
              ressql := datosdb.getTransacSQL('select * from depositos where periodo = ' + '''' + xperiodo + '''' + ' and idprof =  ' + '''' + listSel[i] + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
              ressql.Open;
              while not ressql.Eof do Begin
                entbcos.getDatos(ressql.FieldByName('codbco').AsString);
                list.Linea(0, 0, 'Acreditado en  ' + entbcos.descrip + ', fecha  ' + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString), 1, 'Arial, cursiva, 8', salida, 'S');
...
procedure TTDistribucionObrasSociales.ListarObrasSocialesLiquidadas(xlista: TStringList; xperiodo, xnroliq: String; salida: Char);
...
Begin
  ...
  lista1 := setItemsLiquidados(xperiodo, xnroLiq);
...
procedure TTDistribucionObrasSociales.ListarResumenLisquidacion(xperiodo, xnroLiq: String; salida: char; xrecalcular_montos: Boolean);
...
Begin
  ...
  InicializarMontos(xperiodo, xnroLiq, False);
  retenciones.conectar;
  if (interbase = 'N') then montos.Open;
  ListarMontos('A', xperiodo, xnroliq, salida);
  ListarMontos('B', xperiodo, xnroliq, salida);
  ListarMontos('R', xperiodo, xnroliq, salida);
  ListarMontos('J', xperiodo, xnroliq, salida);
  if (interbase = 'N') then datosdb.closeDB(montos);
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select * from depositos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroliq + '''' + ' order by idprof');
    ressql.Open;
    while not ressql.Eof do Begin
      if totales[1] = 0 then Begin
        if (salida = 'P') or (salida = 'I') then Begin
      ...
      entbcos.getDatos(ressql.FieldByName('codbco').AsString);
      if (salida = 'P') or (salida = 'I') then Begin
        list.Linea(0, 0, ressql.FieldByName('idprof').AsString + '  ' + profesional.nombre, 1, 'Arial, normal, 8', salida, 'N');
        list.Linea(45, list.Lineactual, entbcos.descrip, 2, 'Arial, normal, 8', salida, 'N');
        list.Linea(68, list.Lineactual, utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString), 3, 'Arial, normal, 8', salida, 'N');
        list.Linea(75, list.Lineactual, ressql.FieldByName('observacion').AsString, 4, 'Arial, normal, 8', salida, 'S');
      end;
      if (salida = 'T') then Begin
        list.LineaTxt(CHR(15) + ressql.FieldByName('idprof').AsString + ' ' + Copy(profesional.nombre, 1, 30) + utiles.espacios(31 - Length(TrimRight(Copy(profesional.Nombre, 1, 30)))) +
                      Copy(entbcos.descrip, 1, 25) + utiles.espacios(26 - Length(TrimRight(Copy(entbcos.descrip, 1, 26)))) + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString) + ' ' +
                      Copy(ressql.FieldByName('observacion').AsString, 1, 20) + CHR(18), True); Inc(lineas); if controlarsalto then titulo2(xperiodo);
      end;
      ressql.Next;
    end;
    ressql.Close; ressql.free;
...
procedure TTDistribucionObrasSociales.ListarMontos(xfiltro, xperiodo, xnroliq: String; salida: char);
...
Begin
  ...
  if (interbase = 'S') then begin
    if xfiltro = 'A' then ressql := datosdb.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items = ' + '''' + 'AAA' + '''' + ' order by items');
    if xfiltro = 'B' then ressql := datosdb.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items = ' + '''' + 'BBB' + '''' + ' order by items');
    if xfiltro = 'R' then ressql := datosdb.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items >= ' + '''' + 'R00' + '''' + ' and items <= ' + '''' + 'S99' + '''' + ' order by items');
    if xfiltro = 'J' then ressql := datosdb.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items >= ' + '''' + '000' + '''' + ' and items <= ' + '''' + '999' + '''' + ' order by items');
    ...
    while not ressql.Eof do Begin
      if (xfiltro = 'R') or (xfiltro = 'S') then Begin       // Retenciones
        if (ressql.FieldByName('items').AsString <> idanter) and (totales[1] <> 0) then ListItemsRetenciones(idanter, xperiodo, salida);
        idanter := ressql.FieldByName('items').AsString;
      end;
      if xfiltro = 'J' then Begin       // Ajustes
        if (ressql.FieldByName('items').AsString <> idanter) and (totales[1] <> 0) then ListItemsAjustes(idanter, xperiodo, salida);
        idanter := ressql.FieldByName('items').AsString;
      end;
      totales[1] := totales[1] + ressql.FieldByName('monto').AsFloat;
      ressql.Next;
    end;
    ressql.close; ressql.free;
  ...
  if (interbase = 'N') then montos.IndexFieldNames := 'Idprof;Items';
...
Begin
  if (interbase = 'N') then begin
    ...
    r.Open;
    if (salida = 'P') or (salida = 'I') then Begin
      list.Linea(0, 0, {idanter +' ' +} UpperCase(r.FieldByName('descripaj').AsString), 1, 'Arial, normal, 9', salida, 'N');
      list.Linea(50, list.Lineactual, ':', 2, 'Arial, normal, 9', salida, 'N');
      list.importe(95, list.Lineactual, '', totales[1], 3, 'Arial, normal, 9');
      list.Linea(95, list.Lineactual, '', 4, 'Arial, normal, 9', salida, 'S');
    end;
  ...
  if (interbase = 'S') then begin
    //utiles.msgError('select descripaj from dcprofes where items = ' + '''' + idanter + '''' + ' and periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + copy(xperiodo, 8, 2) + '''');
    s := datosdb.getTransacSQL('select distinct(items), descripaj from dcprofes where items = ' + '''' + idanter + '''' + ' and periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + copy(xperiodo, 8, 2) + '''');
    s.Open;
    if (salida = 'P') or (salida = 'I') then Begin
      list.Linea(0, 0, {idanter + ' - ' +} UpperCase(s.FieldByName('descripaj').AsString), 1, 'Arial, normal, 9', salida, 'N');
      list.Linea(50, list.Lineactual, ':', 2, 'Arial, normal, 9', salida, 'N');
      list.importe(95, list.Lineactual, '', totales[1], 3, 'Arial, normal, 9');
      list.Linea(95, list.Lineactual, '', 4, 'Arial, normal, 9', salida, 'S');
    end;
    if (salida = 'T') then Begin
      list.LineaTxt(Copy(UpperCase(s.FieldByName('descripaj').AsString), 1, 30) + utiles.espacios(31 - Length(TrimRight(Copy(r.FieldByName('descripaj').AsString, 1, 30)))) + '     :' + utiles.espacios(30), False);
      list.ImporteTxt(totales[1], 11, 2, True); Inc(lineas); if controlarsalto then titulo2(xperiodo);
    end;
    s.close; s.free;
    totales[5] := totales[5] + totales[1];
...
procedure TTDistribucionObrasSociales.ListarResumenRetenciones(xperiodo, xnroLiq: String; xlista: TStringList; salida: char);
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' + ' and items >= ' + '''' + 'R00' + '''' + ' and items <= ' + '''' + 'S99' + '''' + ' order by items');
    ressql.open;

    ressql.First; totales[1] := 0; totales[2] := 0;

    while not ressql.Eof do Begin
      if (utiles.verificarItemsLista(xlista, Copy(ressql.FieldByName('items').AsString, 2, 2))) and (ressql.FieldByName('monto').AsFloat > 0) then Begin
        if not retenciones.BuscarCuitDependiente(Copy(ressql.FieldByName('items').AsString, 2, 2), ressql.FieldByName('idprof').AsString) then Begin
          if Copy(ressql.FieldByName('items').AsString, 2, 2) <> idanter then Begin
            if totales[1] <> 0 then ListarTotalRetencion(xperiodo, salida);
            retenciones.getDatos(Copy(ressql.FieldByName('items').AsString, 2, 2));
            if (salida = 'P') or (salida = 'I') then Begin
              if Length(Trim(idanter)) > 0 then list.Linea(0, 0, ' ', 1, 'Arial, negrita, 5', salida, 'S');
              list.Linea(0, 0, 'Retención:  ' + retenciones.Items + ' - ' + retenciones.Descrip, 1, 'Arial, negrita, 8', salida, 'S');
            end;
            ...
            idanter := Copy(ressql.FieldByName('items').AsString, 2, 2);
          end;

          if Copy(ressql.FieldByName('items').AsString, 1, 1) = 'S' then riva := ' - (R.I.)' else riva := '';

          profesional.getDatos(ressql.FieldByName('idprof').AsString);
          if (salida = 'P') or (salida = 'I') then Begin
            list.Linea(0, 0, '    ' + ressql.FieldByName('idprof').AsString, 1, 'Arial, normal, 8', salida, 'N');
            list.Linea(9, list.Lineactual, Copy(profesional.Nombre + riva, 1, 50), 2, 'Arial, normal, 8', salida, 'N');
            list.Linea(45, list.Lineactual, profesional.Nrocuit, 3, 'Arial, normal, 8', salida, 'N');
            list.importe(75, list.Lineactual, '', ressql.FieldByName('total').AsFloat, 4, 'Arial, normal, 8');
            list.importe(95, list.Lineactual, '', ressql.FieldByName('monto').AsFloat, 5, 'Arial, normal, 8');
            list.Linea(95, list.Lineactual, '', 6, 'Arial, normal, 8', salida, 'S');
          end;
          if salida = 'T' then Begin
            list.LineaTxt(CHR(15) + '  ' + ressql.FieldByName('idprof').AsString + ' ' + Copy(profesional.Nombre, 1, 40) + utiles.espacios(41 - Length(TrimRight(Copy(profesional.nombre, 1, 40)))) + ' ' + profesional.Nrocuit, False);
            list.ImporteTxt(ressql.FieldByName('total').AsFloat, 11, 2, False);
            list.ImporteTxt(ressql.FieldByName('monto').AsFloat, 11, 2, True);
            Inc(lineas); if controlarsalto then titulo3(xperiodo);
          end;
          totales[1] := totales[1] + utiles.setNro2Dec(ressql.FieldByName('total').AsFloat);
          totales[2] := totales[2] + utiles.setNro2Dec(ressql.FieldByName('monto').AsFloat);
        end;
      end;
      ressql.Next;
    end;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select * from subtotganancias where periodo = ' + '''' + copy(xperiodo,1, 7) + '''');
    ressql.Open; ressql.First; totales[1] := 0;
    while not ressql.Eof do Begin
      profesional.getDatos(ressql.FieldByName('idprof').AsString);
      if (salida = 'P') or (salida = 'I') then Begin
        list.Linea(0, 0, profesional.codigo + ' - ' + profesional.nombre, 1, 'Arial, normal, 8', salida, 'N');
        list.importe(70, list.Lineactual, '', ressql.FieldByName('monto').AsFloat, 2, 'Arial, normal, 8');
        list.importe(90, list.Lineactual, '', ressql.FieldByName('monto1').AsFloat, 3, 'Arial, normal, 8');
        list.Linea(92, list.Lineactual, '', 4, 'Arial, normal, 8', salida, 'S');
      end;
      if salida = 'T' then Begin
        list.LineaTxt(profesional.codigo + '-' + Copy(profesional.Nombre, 1, 40) + utiles.espacios(41 - Length(TrimRight(Copy(profesional.nombre, 1, 40)))), False);
        list.ImporteTxt(ressql.FieldByName('monto').AsFloat, 12, 2, False);
        list.ImporteTxt(ressql.FieldByName('monto1').AsFloat, 14, 2, True);
        Inc(lineas); if controlarsalto then titulo4(xperiodo);
      end;
      totales[1] := 1;
      ressql.Next;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select max(nrodist) from cabajustesind where periodo = ' + '''' + xperiodo + '''');
    ressql.open;
    if (ressql.RecordCount = 0) then nroLiq := '01' else begin
      nroLiq := ressql.Fields[0].AsString;
    end;
    Result := NroLiq;
  end;
end;
...
procedure TTDistribucionObrasSociales.RegistrarTotal(xperiodo, xidprof, xitems, xnrodist: String; xmonto, xtotal: Real);
...
Begin
  ...
  if (interbase = 'S') then begin
    if ( (xitems = 'AAA') or (xitems = 'BBB') ) then  // Si la liquidación es parcial, no guardamos los totales - 13/08/2014
      if not (__liqTodos) then exit;

    lote.Clear;
    lote.Add('delete from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and items = ' + '''' + xitems + '''' + ' and nrodist = ' + '''' + xnrodist + '''' + ' and idprof = ' + '''' + xidprof + '''');
    lote.Add('insert into montos (periodo, idprof, items, nrodist, monto, total) values (' +
             QuotedStr(copy(xperiodo, 1, 7)) + ', ' +
             QuotedStr(xidprof) + ', ' +
             QuotedStr(xitems) + ', ' +
             QuotedStr(xnrodist) + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(xmonto), ',', '.'), 1, '0') + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(xtotal), ',', '.'), 1, '0') + ')');
    datosdb.TransacSQLBatch(lote);
    lote.clear;
  end;
end;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select periodo from controlliq where periodo =  ' + '''' + xperiodo + '''' + ' and periodoliq = ' + '''' + xperiodoliq + '''' + ' and codos = ' + '''' + xcodos + '''');
    ressql.Open;
    if (ressql.RecordCount > 0) then result := true else result := false;
    ressql.Close; ressql.Free;
  end;
end;
...
Begin
  ...
  if (interbase = 'S') then begin
    lote.Add('delete from controlliq where periodo = ' + QuotedStr(xperiodo) + 'and periodoliq = ' + QuotedStr(xperiodoLiq) + ' and codos = ' + QuotedStr(xcodos));
    lote.Add('insert into controlliq (periodo, periodoliq, codos, porcentaje) values (' +
             QuotedStr(xperiodo) + ', ' + QuotedStr(xperiodoliq) + ', ' + QuotedStr(xcodos) + ', ' +
             utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(floattostr(xporcentaje), ',', '.'), 2, '0') + ')');
  end;
end;

procedure TTDistribucionObrasSociales.BorrarControlLiq(xperiodo, xperiodoliq, xcodos, xnroLiq: String);
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select porcentaje from controlliq where periodoliq = ' + '''' + xperiodoliq + '''' + ' and codos = ' + '''' + xcodos + '''');
    ressql.Open;
    if (ressql.RecordCount > 0) then result := ressql.Fields[0].AsFloat else result := -1;
    ressql.Close; ressql.Free;
  end;
end;
...
procedure TTDistribucionObrasSociales.MarcarConsumoGanancias(xperiodo, xidprof, xnroliq, xconsumido: String);
...
Begin
  ...
  if (interbase = 'S') then begin
    datosdb.TranSQL('update subtotalesliq set consumo = "' + utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(xconsumido, ',', '.'), 1, '0') + '" ' +
                         'where periodo = ' + QuotedStr(xperiodo) + ' and idprof = ' + QuotedStr(xidprof) + ' and nroliq = ' + QuotedStr(xnroliq) );
  end;
end;

function TTDistribucionObrasSociales.verificarConsumoGanancias(xperiodo, xidprof, xnroliq: String): Boolean;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := datosdb.getTransacSQL('select consumo from subtotalesliq where periodo = ' + QuotedStr(xperiodo) + ' and idprof = ' + QuotedStr(xidprof) + ' and nroliq = ' + QuotedStr(xnroliq) );
    ressql.Open;
    if (ressql.FieldByName('consumo').AsString = 'S') then result := true;
    ressql.Close; ressql.Free;
  end;
end;
...
procedure TTDistribucionObrasSociales.FinalizarTransaccion;
begin
  if (interbase = 'S') then begin
    datosdb.TransacSQLBatch(lote);
    lote.Clear;
