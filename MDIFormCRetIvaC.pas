...
type
  TfmSistemFact = class(TForm)
    ...
    skd: TSkinData;
    ActulizacindeDatos1: TMenuItem;
    ParmetrosdelSistema1: TMenuItem;
    ...
    N9: TMenuItem;
    ConfiguracionAccesoRemoto1: TMenuItem;
    ...
    procedure ActulizacindeDatos1Click(Sender: TObject);
    procedure ParmetrosdelSistema1Click(Sender: TObject);
    procedure NomencladorUnicoBioqumico1Click(Sender: TObject);
    ...
    procedure FormCanResize(Sender: TObject; var NewWidth, NewHeight: Integer;
      var Resize: Boolean);
    procedure ConfiguracionAccesoRemoto1Click(Sender: TObject);
    procedure FacturacindeOrdenesOnLine1Click(Sender: TObject);
    procedure ExportarFacturacinSoporteMagntico2Click(Sender: TObject);
  private
...
uses CUtiles, CBDT, ImgForms, NominaPacientes, NominaNomeclador,
  ...
  CFacturacionCCB, NominaDeCategoriasLab, ProgBackup, CUtilidadesArchivos,
  NominaDeProfesionales,  PropiedadesEscritorio, PresupuestoRapido,
  BajarActualizacionesOnLine, Parametros, Nomina_NBU, GestionNBU,
  configaccesoremoto, fmFacturacionOnLine, exportarOrdenesSopMagnetico;
...
procedure TfmSistemFact.ExportarFacturacinSoporteMagntico2Click(
  Sender: TObject);
begin
...
procedure TfmSistemFact.ConfiguracionAccesoRemoto1Click(Sender: TObject);
begin
...
begin
  usuario.desconectar;
  if not Assigned(fmBackup) then Application.CreateForm(TfmBackup, fmBackup);
...
begin
  if not Assigned(fmFactManual) then Begin
    Application.CreateForm(TfmFactManual, fmFactManual);
    fmFactManual.nusuario            := 'LaboratoriosRI';
...
begin
  ...
  if not (skd.Active) and (Length(Trim(skd.SkinFile)) > 0) then
   skd.Active := True;
  if Length(Trim(OpenDialog.FileName)) > 0 then Begin
    AssignFile(archivo, dbs.DirSistema + '\' + Name + '_skinfile.ini');
    Rewrite(archivo);
...
procedure TfmSistemFact.ActulizacindeDatos1Click(Sender: TObject);
begin
...
procedure TfmSistemFact.ParmetrosdelSistema1Click(Sender: TObject);
begin
  Application.CreateForm(TfmParametros, fmParametros);
  fmParametros.ShowModal;
end;

procedure TfmSistemFact.NomencladorUnicoBioqumico1Click(Sender: TObject);
begin
...
procedure TfmSistemFact.IncluiryExcluirCdigosNomenclatura1Click(
  Sender: TObject);
begin
