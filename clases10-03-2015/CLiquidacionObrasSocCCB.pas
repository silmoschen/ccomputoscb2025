...
TTDistribucionObrasSociales = class(TObject)
  ...
  function    setItemsAjustesIndividualesIB(xperiodo, xcodos, xnroLiq: String): TIBQuery;
  function    setItemsAjustesIndividualesProfesional(xperiodo, xidprof, xnroLiq: String): TQuery;
  function    setItemsAjustesIndividualesProfesionalIB(xperiodo, xidprof, xnroLiq: String): TIBQuery;
  ...
  function    setItemsDCProfIB(xperiodo, xnroLiq: String): TIBQuery; overload;
  function    setItemsDCProfIB(xperiodo, xidprof, xnroLiq: String): TIBQuery; overload;
 ...
 private
  ...
  totganancias, monto_g, __monto: Real;
...
begin
  ...
  if (interbase = 'S') then begin
    ExisteLiquidacion := False;
    ressql := ffirebird.getTransacSQL('select count(*) from cabajustesind where periodo = ' + '''' + xperiodo + '''' + ' and codos = ' + '''' + xcodos + '''' + ' and perliq = ' + '''' + xperliq + '''');
    ressql.Open;
...
begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select * from cabajustesind where periodo = ' + QuotedStr(xperiodo) + ' and codos = ' + QuotedStr(xcodos) +
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select * from cabajustesind where periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroliq + '''');
    ressql.Open;
    l := TStringList.Create;
...
function TTDistribucionObrasSociales.setItemsAjustesIndividualesIB(xperiodo, xcodos, xnroLiq: String): TIBQuery;
// Objetivo...: Devolver ajustes de una obra social
begin
  if (interbase = 'S') then Result := ffirebird.getTransacSQL('SELECT * FROM detajindiv WHERE periodo = ' + '"' + xperiodo + '"' + ' AND codos = ' + '"' + xcodos + '"' + ' and nrodist = ' + '"' + xnroliq + '"');
end;
...
function TTDistribucionObrasSociales.setItemsAjustesIndividualesProfesionalIB(xperiodo, xidprof, xnroliq: String): TIBQuery;
// Objetivo...: Devolver Ajustes de un Profesional
begin
  if (interbase = 'S') then Result := ffirebird.getTransacSQL('SELECT * FROM detajindiv WHERE perliq = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND nrodist = ' + '"' + xnroliq + '"');
end;
...
begin
  ...
  if (interbase = 'S') then begin
    ...
    ressql := ffirebird.getTransacSQL('select * from detajindiv where perliq = ' + '''' + xperiodo + '''' + ' AND idprof = ' + '''' + xidprof + '''' + ' and nrodist = ' + '"' + xnroliq + '"');
    ressql.Open;
    while not ressql.Eof do Begin
...
Begin
  ...
  if (interbase = 'S') then begin
    monto := 0;
    ressql := ffirebird.getTransacSQL('select * from subtotganancias where periodo = ' + QuotedStr(Copy(xperiodo, 1, 2) + Copy(xperiodo, 4, 4)) + ' and idprof = ' + QuotedStr(xidprof));
    ressql.Open;
    monto := ressql.FieldByName('monto').AsFloat;
    ressql.Close; ressql.Free;

    ressql := ffirebird.getTransacSQL('select * from subtotganancias where periodo = ' + QuotedStr(Copy(xperiodo, 1, 2) + Copy(xperiodo, 4, 4)));
    ressql.Open;
    // Ahora Buscar los CUIT dependientes, para unificar el monto
    l := retenciones.setCuitProfesional(xidprof);
    For i := 1 to l.Count do Begin
...
begin
  ...
  if (interbase = 'S') then begin
    l := TStringList.Create;
    ressql := ffirebird.getTransacSQL('select * from depositos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' +xnroliq + '''');
    ressql.Open;
    while not ressql.Eof do Begin
...
Begin
  ...
  if (interbase = 'S') then ffirebird.TransacSQL('delete from dcprofes where tipomov = ' + '"' + 'F' + '"' + ' and periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroliq + '''');
end;
...
function  TTDistribucionObrasSociales.setItemsDCProfIB(xperiodo, xnroLiq: String): TIBQuery;
begin
  Result := ffirebird.getTransacSQL('SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' AND nrodist = ' + '''' + xnroliq + '''' + ' ORDER BY Tipomov, Idprof, Items');
end;
...
Begin
  ...
  if (interbase = 'S') then begin
    debitoscreditos.conectar;
    ressql := ffirebird.getTransacSQL('select * from dcprofes where idprof = ' + '''' + xidprof + '''' + ' and periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroliq + '''' + ' and importeaj <> 0');  // importe <> 0 10/03/2015
    ressql.Open;
...
function  TTDistribucionObrasSociales.setItemsDCProfIB(xperiodo, xidprof, xnroLiq: String): TIBQuery;
begin
  Result := ffirebird.getTransacSQL('SELECT * FROM dcprofes WHERE Periodo = ' + '"' + xperiodo + '"' + ' AND idprof = ' + '"' + xidprof + '"' + ' AND nrodist = ' + '"' + xnroliq + '"'  + ' AND tipomov = ' + '"M"' + ' ORDER BY Idprof, Items');
end;
...
var
  ...
  begin
    ...
    if (interbase = 'S') then begin
      ressql := ffirebird.getTransacSQL('select * from depositos where periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + xidprof + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
      ressql.Open;
      while not ressql.Eof do Begin
...
Begin
  ...
  if listSel = Nil then utiles.msgError('No hay Profesional/Laboratorio Seleccionado ...!') else Begin
    ...
    if (interbase = 'S') then begin
      ffirebird.TransacSQL('delete from montosret where idprof = ' + '''' + listSel[i] + '''' + ' and periodo = ' + '''' + xperiodo + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
    end;
      ...
      if totales[1] < 0 then list.Linea(0, 0, ' ', 1, 'Arial, normal, 8', salida, 'S') else Begin
        ...
        For ix := 1 to lista2.Count do Begin
          ...
          if (interbase = 'S') then begin
             ...
             //utiles.msgError('4');
             __monto := 0;
             ressql := ffirebird.getTransacSQL('select retencion from montosret where periodo = ' + '''' + Copy(lista1.Strings[ix-1], 14, 7) + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and codos = ' + '''' + Copy(lista2.Strings[ix-1], 14, 6) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
             ressql.open;
              ...
              if (salida = 'P') or (salida = 'I') or (salida = 'T') then Begin
                ...
                if StrToFloat(Trim(Copy(lista3.Strings[ix-1], p2 + 2, (p3 - (p2+2))))) = 0 then Begin
                  ...
                  if not retenciones.setItemsDiferencial(Copy(lista3.Strings[ix-1], 1, 2)) then Begin
                    ...
                    if (interbase = 'S') then begin
                      ressql := ffirebird.getTransacSQL('select * from montosret where periodo = ' + '''' + xperiodo + '''' + ' and idprof = ' + '''' + listSel[i] + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
                      ressql.Open;
                      while not ressql.eof do begin
        ...
        if (salida = 'P') or (salida = 'I') then Begin
          ...
          if not (LiquidarRetencionesSeparadas) then begin
            ...
            if (interbase = 'S') then begin
              ressql := ffirebird.getTransacSQL('select * from depositos where periodo = ' + '''' + xperiodo + '''' + ' and idprof =  ' + '''' + listSel[i] + '''' + ' and nrodist = ' + '''' + xnroLiq + '''');
              ressql.Open;
              while not ressql.Eof do Begin
                entbcos.getDatos(ressql.FieldByName('codbco').AsString);
                list.Linea(0, 0, 'Acreditado en  ' + entbcos.descrip + ', fecha  ' + utiles.sFormatoFecha(ressql.FieldByName('fecha').AsString), 1, 'Arial, cursiva, 8', salida, 'S');
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select * from depositos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroliq + '''' + ' order by idprof');
    ressql.Open;
    while not ressql.Eof do Begin
      if totales[1] = 0 then Begin
        if (salida = 'P') or (salida = 'I') then Begin
...
Begin
  ...
  if (interbase = 'S') then begin
    if xfiltro = 'A' then ressql := ffirebird.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items = ' + '''' + 'AAA' + '''' + ' order by items');
    if xfiltro = 'B' then ressql := ffirebird.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items = ' + '''' + 'BBB' + '''' + ' order by items');
    if xfiltro = 'R' then ressql := ffirebird.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items >= ' + '''' + 'R00' + '''' + ' and items <= ' + '''' + 'S99' + '''' + ' order by items');
    if xfiltro = 'J' then ressql := ffirebird.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' +  ' and items >= ' + '''' + '000' + '''' + ' and items <= ' + '''' + '999' + '''' + ' order by items');
...
Begin
  ...
  if (interbase = 'S') then begin
    ...
    s := ffirebird.getTransacSQL('select distinct(items), descripaj from dcprofes where items = ' + '''' + idanter + '''' + ' and periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + copy(xperiodo, 8, 2) + '''');
    s.Open;
    if (salida = 'P') or (salida = 'I') then Begin
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select * from montos where periodo = ' + '''' + copy(xperiodo, 1, 7) + '''' + ' and nrodist = ' + '''' + xnroLiq + '''' + ' and items >= ' + '''' + 'R00' + '''' + ' and items <= ' + '''' + 'S99' + '''' + ' order by items');
    ressql.open;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select * from subtotganancias where periodo = ' + '''' + copy(xperiodo,1, 7) + '''');
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select max(nrodist) from cabajustesind where periodo = ' + '''' + xperiodo + '''');
    ressql.open;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select periodo from controlliq where periodo =  ' + '''' + xperiodo + '''' + ' and periodoliq = ' + '''' + xperiodoliq + '''' + ' and codos = ' + '''' + xcodos + '''');
    ressql.Open;
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select porcentaje from controlliq where periodoliq = ' + '''' + xperiodoliq + '''' + ' and codos = ' + '''' + xcodos + '''');
    ressql.Open;
...
Begin
  ...
  if (interbase = 'S') then begin
    ffirebird.TransacSQL('update subtotalesliq set consumo = "' + utiles.sLlenarIzquierda(utiles.StringRemplazarCaracteres(xconsumido, ',', '.'), 1, '0') + '" ' +
...
Begin
  ...
  if (interbase = 'S') then begin
    ressql := ffirebird.getTransacSQL('select consumo from subtotalesliq where periodo = ' + QuotedStr(xperiodo) + ' and idprof = ' + QuotedStr(xidprof) + ' and nroliq = ' + QuotedStr(xnroliq) );
    ressql.Open;
